...
let rec remove_dir dir =
  Lwt_stream.iter_p (remove dir) (Lwt_unix.files_of_directory dir)
  >>= fun () -> Lwt_unix.rmdir dir

and remove dir name =
  if name = "." || name = ".." then Lwt.return_unit
  else
...
let with_temp_dir f =
  let rec get_dir () =
    ...
    @@ function
    | Unix.Unix_error (Unix.EEXIST, _, _) -> get_dir () | e -> raise e
  in
  get_dir ()
  >>= fun dir ->
  Lwt.catch
...
let get_grade ?callback ?timeout ?dirname exo solution =
  ...
  let divert name chan cb =
    ...
    fun () -> Toploop_unix.stop_channel_redirection redirection
  in
  Lwt.wrap
  @@ fun () ->
