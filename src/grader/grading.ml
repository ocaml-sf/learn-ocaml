...
let () =
  Location.register_error_of_exn (fun exn ->
      ...
      | Some msg ->
          Some {Location.loc = Location.none; sub = []; msg; if_highlight = msg}
      | None -> None )

let internal_error name err = raise (Internal_error (name, err))

let user_code_error err = raise (User_code_error err)

let get_grade ?callback ?timeout ?(dirname = "") ~divert
  ...
  let ppf_answer = Format.formatter_of_buffer outcomes_buffer in
  let stderr_buffer = Buffer.create 503 in
  let stdout_buffer = Buffer.create 503 in
  let flush_stderr =
    ref (divert "ERR" stderr (Buffer.add_string stderr_buffer))
  in
  let flush_stdout =
    ref (divert "OUT" stdout (Buffer.add_string stdout_buffer))
  in
  let callback =
    match callback with
    | None -> None
    | Some callback ->
        Some
          (fun msg ->
            ...
            flush_stdout :=
              divert "OUT" stdout (Buffer.add_string stdout_buffer) )
  in
  let set_progress =
    match callback with
    | None -> fun _ -> ()
    | Some set_progress -> set_progress
  in
  let handle_error ?(warn = fun _ -> ()) fail = function
    | Toploop_ext.Ok (s, w) ->
        warn w;
        if not s then (
          !flush_stderr ();
          ...
          let msg =
            String.concat "\n"
              (List.map Buffer.contents
                 [stderr_buffer; stdout_buffer; outcomes_buffer])
          in
          fail {Toploop_ext.msg; locs = []; if_highlight = msg} )
    | Toploop_ext.Error (err, w) ->
        warn w; !flush_stderr (); !flush_stdout (); fail err
  in
  let result =
    try
      ...
      @@ Toploop_ext.use_mod_string ~print_outcome ~ppf_answer
           ~modname:"Solution"
      ...
      let get_result =
        Introspection.create_ref "results" [%ty: Learnocaml_report.t option]
          None
      in
      Introspection.register_callback "set_progress" [%ty: string] set_progress;
      ...
      let () =
        ...
        let rec load_dependencies signatures = function
          ...
          | file :: fs -> (
              let path = File.key file and content = decipher file exo in
              let modname =
                String.capitalize_ascii @@ Filename.remove_extension
                @@ Filename.basename path
              in
              match Filename.extension path with
              | ".mli" ->
                  load_dependencies ((modname, content) :: signatures) fs
              | ".ml" ->
                  ...
                  in
                  ( handle_error
                      (internal_error [%i "while loading user dependencies"])
                  @@
                  match included with
                  | true ->
                      Toploop_ext.use_string ~print_outcome ~ppf_answer
              ...
              | _ ->
                  failwith
                    ( "uninterpreted dependency \"" ^ path
                    ^ "\", file extension expected : .ml or .mli" ) )
        in
        load_dependencies [] files
      in
      handle_error (internal_error [%i "while testing your solution"])
      @@ Toploop_ext.use_string ~print_outcome ~ppf_answer
           ~filename:(file "test.ml")
