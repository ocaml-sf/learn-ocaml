...
let get_grade
  ...
  let onmessage (ev : Browser_json.Repr.value Worker.messageEvent Js.t) =
    let json = ev##data in
    begin match Browser_json.Json_encoding.destruct from_worker_enc json with
  ...
  fun solution ->
    ...
    let json = Browser_json.Json_encoding.construct to_worker_enc req in
