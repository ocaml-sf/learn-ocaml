...
module Make (Params : sig
  ...
  let set_result report = Params.results := Some report
  ...
  module Ast_checker = struct
    ...
    let ast_check f ?(on_expression = fun _ -> []) ?(on_pattern = fun _ -> [])
        ?(on_structure_item = fun _ -> []) ?(on_external = fun _ -> [])
        ?(on_include = fun _ -> []) ?(on_open = fun _ -> [])
        ?(on_module_occurence = fun _ -> [])
        ?(on_variable_occurence = fun _ -> [])
        ?(on_function_call = fun _ -> []) arg =
      ...
      let treat_variable ({Location.txt; _} as ident) =
        ...
        | _ -> add @@ on_variable_occurence (name ident)
      in
      let expr mapper expr =
        ...
        | {pexp_desc = Pexp_let (Asttypes.Nonrecursive, pvs, iexpr); _} ->
            ...
            List.iter
              (fun {pvb_expr; _} -> ignore (mapper.expr mapper pvb_expr))
              pvs;
            List.iter
              (fun {pvb_pat; _} -> ignore (mapper.pat mapper pvb_pat))
              pvs;
            ignore (mapper.expr mapper iexpr);
            variables := before;
      ...
      let structure_item mapper structure_item =
        ...
        | {pstr_desc = Pstr_recmodule mbs; _} ->
            ...
            List.iter
              (fun {pmb_name; _} ->
                modules := pmb_name.Location.txt :: !modules )
              mbs;
            let before = !modules in
            List.iter
              (fun {pmb_expr; _} ->
        ...
        | {pstr_desc = Pstr_primitive p; _} as si ->
            ...
            structure_item
        | si -> default_mapper.structure_item mapper si
      in
      let typ mapper typ =
        match typ with
      ...
      let case mapper ({pc_lhs; pc_guard; pc_rhs} as case) =
        ...
        ( match pc_guard with
        | Some pc_guard -> ignore (mapper.expr mapper pc_guard)
        | None -> () );
      ...
      in
      let mapper =
        {default_mapper with case; expr; structure_item; pat; typ; module_expr}
      in
      f (mapper.expr mapper, mapper.structure mapper) arg;
    ...
    let ast_check_structure =
      ast_check
      @@ fun (_, check_structure) structure ->
      let structure =
        ast_location_stripper.Ast_mapper.structure ast_location_stripper
          structure
      in
    ...
    let forbid_syntax n =
      ...
      fun _ ->
        if !already then []
          ...
          Learnocaml_report.
            [ Message
                ([Text "The "; Code n; Text " syntax is forbidden"], Failure)
            ] )

    let require_syntax n =
      let already = ref false in
      fun _ ->
        if !already then []
        else (
          ...
          Learnocaml_report.
            [ Message
                ( [ Text "The "
                  ; Code n
                  ; Text " syntax has been found, as expected" ]
                , Success 5 ) ] )
    ...
    let require k pr _ =
      ...
      fun n ->
        if !already then []
          ...
          Learnocaml_report.
            [ Message
                ([Text "Found "; Text k; Text " "; Code (pr n)], Success 5) ] )
    ...
    let require_expr name expr = require name print_exp (stripper expr)
    ...
    let ast_sanity_check ?(modules = []) ast cb =
      ...
      in
      let sanity_report =
        ast_check_structure ~on_external:(forbid_syntax "external")
          ~on_module_occurence:(forbid "module" (fun name -> name) modules)
          ast
        |> List.sort_uniq compare
      in
      if snd (Learnocaml_report.result sanity_report) then sanity_report
      else cb ()
      ...
      let rec findlet = function
        | [] ->
            [ Message
                ( [ Text "I could not find "
                  ...
                  ; Text "."
                  ; Break
                  ; Text "Check that it is defined as a simple "
                  ; Code "let"
                  ; Text " at top level." ]
                , Failure ) ]
        | {pstr_desc = Pstr_value (_, bds); _} :: rest ->
            let rec findvar = function
              | [] -> findlet rest
              | { pvb_pat = {ppat_desc = Ppat_var {Location.txt; _}; _}
                ; pvb_expr; _ }
                :: _
                when txt = name ->
                  Message
  ...
  module Test_functions_types = struct
    ...
    let compatible_type ~expected:exp got =
      ...
      ; ( match Introspection.compatible_type exp ("Code." ^ got) with
        | Introspection.Absent -> Message ([Text "Type not found"], Failure)
        | Introspection.Incompatible msg -> Message ([Text msg], Failure)
    ...
    let existing_type ?(score = 1) name =
      ...
      try
        let path =
          Env.lookup_type
            Longident.(parse ("Code." ^ name))
            !Toploop.toplevel_env
        in
        let _ = Env.find_type path !Toploop.toplevel_env in
        ( true
        , [Message ([Text "Type"; Code name; Text "found"], Success score)] )
      with Not_found ->
        (false, [Message ([Text "type"; Code name; Text "not found"], Failure)])
      ...
      try
        ...
        | {Types.type_kind = Types.Type_abstract; Types.type_manifest = None; _}
          ->
            ( true
            , [ Message
                  ( [Text "Type"; Code name; Text "is abstract as expected."]
                  , Success score ) ] )
        | {Types.type_kind = _; type_private = Asttypes.Private; _}
          when allow_private ->
            ( true
            , [ Message
                  ( [ Text "Type"
                    ; Code name
                    ; Text "is private, I'll accept that :-)." ]
                  , Success score ) ] )
        | {Types.type_kind = _; _} ->
            ( false
            , [ Message
                  ( [Text "Type"; Code name; Text "should be abstract!"]
                  , Failure ) ] )
      with Not_found ->
        ( false
        , [Message ([Text "Type"; Code name; Text "not found."], Failure)] )
    ...
    let test_student_code ty cb =
      ...
      | Introspection.Incompatible msg ->
          [ Message
              ( [ Text "Your code doesn't match the expected signature."
                ; Break
                ; Code msg
                (* TODO: hide or fix locations *) ]
              , Failure ) ]
    ...
    let test_module_property ty name cb =
      ...
      | Introspection.Incompatible msg ->
          [ Message
              ( [ Text "Module"
                ; Code name
                ; Text "doesn't match the expected signature."
                ; Break
                ; Code msg
                (* TODO: hide or fix locations *) ]
              , Failure ) ]
  ...
  module Tester = struct
    ...
    let test_generic eq canon ty va vb =
      ...
      if eq (canon va) (canon vb) then
        ...
        in
        Learnocaml_report.[Message ([Text txt_msg; Code code_msg], Success 1)]
      else
        let txt_msgs =
          match va with
          | Ok v -> Learnocaml_report.[Text "Wrong value"; Code (to_string v)]
          | Error (Failure s) when s = "EXCESS" ->
              Learnocaml_report.
                [Text "Your code exceeded the output buffer size limit."]
          ...
          | Error (Timeout limit) ->
              Learnocaml_report.
                [ Text
                    (Format.sprintf
                       "Your code exceeded the time limit of %d seconds." limit)
                ]
          | Error exn ->
              Learnocaml_report.
                [Text "Wrong exception"; Code (exn_to_string exn)]
        in
        Learnocaml_report.[Message (txt_msgs, Failure)]
    ...
    let test_ignore ty va vb =
      ...
      | Error Stack_overflow, _ ->
          Learnocaml_report.
            [Message ([Text "Stack overflow. Too many recursions?"], Failure)]
      | Error _, Error _ -> []
      | Error exn, Ok _ ->
          Learnocaml_report.
            [ Message
                ( [Text "Unexpected exception"; Code (exn_to_string exn)]
                , Failure ) ]

    let test ty va vb = test_generic ( = ) (fun x -> x) ty va vb

    let test_eq eq = test_generic eq (fun x -> x)

    let test_eq_ok eq =
      let eq ra rb =
        match (ra, rb) with Ok a, Ok b -> eq a b | _ -> ra = rb
      in
      test_generic eq (fun x -> x)

    let test_eq_exn eq =
      let eq ra rb =
        match (ra, rb) with Error a, Error b -> eq a b | _ -> ra = rb
      in
      test_generic eq (fun x -> x)

    let test_canon canon = test_generic ( = ) canon
      ...
      if chars = [] then fun s -> [s]
        ...
        fun s ->
          ...
          let rec loop acc i j =
            ...
            else if pattern.(Char.code s.[j]) then
    ...
    let trimmer chars =
      if chars = [] then fun s -> s
        ...
        fun s ->
          let len = String.length s in
          let rec start i =
            if i >= len then stop i (len - 1)
            else if pattern.(Char.code s.[i]) then start (i + 1)
            else stop i (len - 1)
          and stop i j =
            if j < 0 then (i, 0)
            else if pattern.(Char.code s.[j]) then stop i (j - 1)
            else (i, j + 1)
          in
          let i, j = start 0 in
        ...
        fun s ->
          ...
          let rec drop i m =
            if i = len then m
              ...
              let c = s.[i] in
              if pattern.(Char.code c) then drop (i + 1) m
              else (
                Bytes.set buf m c;
                drop (i + 1) (m + 1) )
          in
    ...
    let io_test_equals ?(trim = []) ?(drop = []) =
      ...
      fun got expected ->
        ...
        else [Message ([Text "Unexpected output"; Output got], Failure)]

    let io_test_items ?(split = []) ?(trim = []) ?(drop = [])
      ...
      fun sgot sexpected ->
        ...
        let expected =
          if skip_empty then List.filter (( <> ) "") expected else expected
        in
        let rec test_items = function
          ...
          | got :: gots, expected :: expecteds ->
              ...
              else test_items (gots, expecteds)
          | _, _ -> [Message ([Text "Unexpected output"; Output sgot], Failure)]
        in
        test_items (got, expected)

    let io_test_lines ?(trim = []) ?(drop = []) ?(skip_empty = false)
        ?(test_line = io_test_equals ~trim:[] ~drop:[]) got expected =
      io_test_items ~split:['\n'] ~trim ~drop ~skip_empty ~test_item:test_line
        got expected
  end
  ...
  module Mutation = struct
    ...
    let arg_mutation_test_callbacks ?(test = test) ~dup ~blit ty =
      ...
      let got = ref None in
      let exp = ref None in
      let before_reference a = sam := Some (dup a) in
      let before_user a =
        exp := Some (dup a);
        got := Some a;
        match !sam with None -> () | Some src -> blit src a
      in
        ...
        let report =
          ...
          | Ok _, Ok _ ->
              let got =
                match !got with
                | Some g -> g
                | None -> invalid_arg "arg_mutation_test_callbacks"
              in
              let exp =
                match !exp with
                | Some e -> e
          ...
          | Error ea, Error eb -> test ty (Error ea) (Error eb)
        in
  ...
  module Test_functions_generic = struct
    ...
    let sigalrm_handler time = Sys.Signal_handle (fun _ -> raise (Timeout time))
    ...
    let run_timeout ?time v =
      ...
      | Some time, _ | None, Some time -> run_timeout ~time v
      | None, None -> v ()
    ...
    let result v =
      match exec v with Ok (v, _, _) -> Ok v | Error exn -> Error exn

    (*----------------------------------------------------------------------------*)

    let verify ?(test_stdout = fun _ -> []) ?(test_stderr = fun _ -> [])
    ...
    let expect ?(test = test) ?(test_stdout = io_test_ignore)
        ?(test_stderr = io_test_ignore) ?(pre = fun _ -> ())
        ?(post = fun _ _ -> []) ty va vb =
      let vb = exec vb in
      ...
      | Ok (va, outa, erra), Ok (vb, outb, errb) ->
          ...
          report @ stdout_report @ stderr_report @ post_report
      | Ok (va, _, _), Error exnb -> test ty (Ok va) (Error exnb)
      | Error exna, Ok (vb, _, _) -> test ty (Error exna) (Ok vb)
      | Error exna, Error exnb -> test ty (Error exna) (Error exnb)
    ...
    let[@ocaml.deprecated "Use ty_of_fun_ty instead."] ty_of_prot =
      ty_of_fun_ty
    ...
    let lookup ty ?display_name name =
      let display_name =
        match display_name with None -> name | Some name -> name
      in
      let open Learnocaml_report in
      let res =
        match Introspection.get_value name ty with
        ...
        | Introspection.Absent ->
            `Unbound
              ( name
              , [Message ([Text "Cannot find "; Code display_name], Failure)]
              )
        | Introspection.Incompatible msg ->
            `Unbound
              ( name
    ...
    let lookup_student ty name =
      ...
      let res =
        ...
        | Introspection.Present v ->
            let msg =
              [ Message
                  ( [Text "Found"; Code name; Text "with compatible type."]
                  , Informative ) ]
            in
            `Found (name, msg, v)
        | Introspection.Absent ->
    ...
    let lookup_solution ty name =
      let open Learnocaml_report in
      let res =
        match Introspection.get_value ("Solution." ^ name) ty with
        | Introspection.Present v -> `Found (name, [], v)
        | Introspection.Absent ->
            `Unbound
              ( name
              , [ Message ([Text "Looking for "; Code name], Informative)
        ...
        | Introspection.Incompatible msg ->
            `Unbound
              ...
              , [ Message ([Text "Looking for "; Code name], Informative)
                ; Message
                    ([Text "Solution is wrong!"; Break; Code msg], Failure) ]
              )
      in
      fun () -> res
    ...
    let run_test ?(before = fun _ -> ()) ~after name prot tests for_case =
      let before args () = before args in
      let ty = Fun_ty.ty_of_fun_ty prot in
      let for_casel case =
        let args, ret = case () in
        let code =
          Format.asprintf "@[<hv 2>%s%a@]" name (FunTyAux.print prot) args
        in
      ...
      in
      List.flatten @@ List.map for_casel tests

    let test_function_generic ?test ?test_stdout ?test_stderr ?before
        ?(after = fun _ _ _ -> []) prot uf tests =
      test_value uf
      @@ fun ruf ->
      let for_case pre post args ret_ty =
        expect ~pre ~post ?test ?test_stdout ?test_stderr ret_ty (fun () ->
            Fun_ty.apply ruf args )
      in
      run_test ?before ~after (name uf) prot tests for_case

    let test_function_generic_postcond ?test_stdout ?test_stderr ?before
        ?(after = fun _ _ -> []) test name prot tests =
      let for_case pre post args =
        verify ~pre ~post ?test_stdout ?test_stderr (test args)
      in
      run_test ?before ~after name prot tests for_case

    let test_function ?test ?test_stdout ?test_stderr ?before ?after prot uf
        tests =
      test_function_generic ?test ?test_stdout ?test_stderr ?before ?after prot
        uf
        (List.map (fun x () -> x) tests)

    let make_tests ?gen ?sampler ?(before_reference = fun _ -> ()) prot rf
        tests =
      let gen =
        match gen with Some n -> n | None -> max 5 (10 - List.length tests)
      in
      let tests =
        match gen with
        ...
        | _ ->
            ...
            in
            let rec make i = if i <= 0 then [] else sampler :: make (i - 1) in
            List.map (fun x () -> x) tests @ make gen
      in
      List.map
        (fun a () ->
          let a = a () in
          (a, fun () -> before_reference a; Fun_ty.apply rf a) )
        tests

    let test_function_against_generic ?gen ?test ?test_stdout ?test_stderr
      ...
      test_function_generic ?test ?test_stdout ?test_stderr ?before:before_user
        ?after prot uf tests

    let test_function_against_generic_postcond ?gen ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler test name prot rf tests
        =
      test_value rf
      @@ fun rf ->
      let tests = make_tests ?gen ?sampler ?before_reference prot rf tests in
      test_function_generic_postcond ?test_stdout ?test_stderr
        ?before:before_user ?after test name prot tests

    let test_function_against ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler prot uf rf tests =
      test_function_against_generic ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler prot uf rf tests

    let test_function_against_solution ?gen ?test ?test_stdout ?test_stderr
  ...
  module Test_functions_ref_var = struct
    ...
    let test_ref ty got exp =
      let open Learnocaml_report in
      let mk_txt str = [Text str; Code (print_with ty !got)] in
      if !got = exp then [Message (mk_txt "Correct value", Success 1)]
      else [Message (mk_txt "Wrong value", Failure)]

    let test_variable ty name r =
      test_value (lookup_student ty name)
      @@ fun v -> expect ~test ty (fun () -> v) (fun () -> r)

    let test_variable_property ty name cb =
      test_value (lookup_student ty name) cb

    let test_variable_against_solution ty name =
      test_value (lookup_solution ty name)
      @@ fun sol -> test_variable ty name sol
  end
  ...
  module Test_functions_function = struct
    ...
    let function_1_adapter_pre sampler ty =
      let pre = function
        | None -> fun _ -> ()
        | Some pre -> Fun_ty.apply_args_1 pre
      in
      let sampler =
        match sampler with
        | None -> None
        | Some sampler -> Some (fun () -> Fun_ty.last (sampler ()))
      in
    ...
    let test_function_1 ?test ?test_stdout ?test_stderr ?before ?after ty name
        tests =
      let tests =
        List.map
          (fun (x, r, out, err) ->
            ( Fun_ty.last x
            , fun () -> output_string stdout out; output_string stderr err; r
            ) )
          tests
      in
      let after, pre, _, prot = function_1_adapter after None ty in
      test_function ?test ?test_stdout ?test_stderr ~before:(pre before) ~after
        prot (lookup_student ty name) tests

    let test_function_1_against ?gen ?test ?test_stdout ?test_stderr
      ...
      test_function_against ?gen ?test ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler prot (lookup_student ty name) (found name rf) tests

    let test_function_1_against_solution ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name tests =
      let tests = List.map (fun x -> Fun_ty.last x) tests in
      let after, pre, sampler, prot = function_1_adapter after sampler ty in
      test_function_against ?gen ?test ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler prot (lookup_student ty name) (lookup_solution ty name)
        tests

    let test_function_1_against_postcond ?gen ?test_stdout ?test_stderr
      ...
      in
      let pre, sampler, prot = function_1_adapter_pre sampler ty in
      test_function_against_generic_postcond ?gen ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler (Fun_ty.apply_args_1 test) name prot
        (lookup_student ty name) tests
    ...
    let function_2_adapter_pre sampler ty =
      let pre = function
        | None -> fun _ -> ()
        | Some pre -> Fun_ty.apply_args_2 pre
      in
      let sampler =
        match sampler with
        | None -> None
    ...
    let function_2_adapter after sampler ty =
      let after =
        match after with
        | None -> fun _ _ _ -> []
        | Some after -> Fun_ty.apply_args_2 after
      in
      let pre, sampler, prot = function_2_adapter_pre sampler ty in
      (after, pre, sampler, prot)

    let test_function_2 ?test ?test_stdout ?test_stderr ?before ?after ty name
        tests =
      let tests =
        List.map
          (fun (x, y, r, out, err) ->
            ( Fun_ty.arg x @@ Fun_ty.last y
            , fun () -> output_string stdout out; output_string stderr err; r
            ) )
          tests
      in
    ...
    let test_function_2_against ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name rf tests =
      let tests =
        List.map (fun (x, y) -> Fun_ty.arg x @@ Fun_ty.last y) tests
      in
    ...
    let test_function_2_against_solution ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name tests =
      let tests =
        List.map (fun (x, y) -> Fun_ty.arg x @@ Fun_ty.last y) tests
      in
    ...
    let test_function_2_against_postcond ?gen ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler test ty name tests =
      let tests =
        List.map (fun (x, y) -> Fun_ty.arg x @@ Fun_ty.last y) tests
      in
      ...
      test_function_against_generic_postcond ?gen ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler (Fun_ty.apply_args_2 test) name prot
        (lookup_student ty name) tests
    ...
    let function_3_adapter_pre sampler ty =
      let pre = function
        | None -> fun _ -> ()
        | Some pre -> Fun_ty.apply_args_3 pre
      in
      let sampler =
        match sampler with
        | None -> None
        | Some sampler ->
            Some
              (fun () ->
                ...
                Fun_ty.arg a (Fun_ty.arg b (Fun_ty.last c)) )
      in
      let arg1_ty, ret_ty = Ty.domains ty in
      let arg2_ty, ret_ty = Ty.domains ret_ty in
      let arg3_ty, ret_ty = Ty.domains ret_ty in
      let prot =
        Fun_ty.arg_ty arg1_ty @@ Fun_ty.arg_ty arg2_ty
        @@ Fun_ty.last_ty arg3_ty @@ ret_ty
      in
      (pre, sampler, prot)
    ...
    let function_3_adapter after sampler ty =
      let after =
        match after with
        | None -> fun _ _ _ -> []
        | Some after -> Fun_ty.apply_args_3 after
      in
      let pre, sampler, prot = function_3_adapter_pre sampler ty in
      (after, pre, sampler, prot)

    let test_function_3 ?test ?test_stdout ?test_stderr ?before ?after ty name
        tests =
      let tests =
        List.map
          (fun (w, x, y, r, out, err) ->
            ( Fun_ty.arg w @@ Fun_ty.arg x @@ Fun_ty.last y
            , fun () -> output_string stdout out; output_string stderr err; r
            ) )
          tests
      in
    ...
    let test_function_3_against ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name rf tests =
      let tests =
        List.map
          (fun (w, x, y) -> Fun_ty.arg w @@ Fun_ty.arg x @@ Fun_ty.last y)
          tests
      in
    ...
    let test_function_3_against_solution ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name tests =
      let tests =
        List.map
          (fun (w, x, y) -> Fun_ty.arg w @@ Fun_ty.arg x @@ Fun_ty.last y)
    ...
    let test_function_3_against_postcond ?gen ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler test ty name tests =
      let tests =
        List.map
          (fun (w, x, y) -> Fun_ty.arg w @@ Fun_ty.arg x @@ Fun_ty.last y)
      ...
      test_function_against_generic_postcond ?gen ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler (Fun_ty.apply_args_3 test) name prot
        (lookup_student ty name) tests
    ...
    let function_4_adapter_pre sampler ty =
      let pre = function
        | None -> fun _ -> ()
        | Some pre -> Fun_ty.apply_args_4 pre
      in
      let sampler =
        match sampler with
        | None -> None
        | Some sampler ->
            Some
              (fun () ->
                ...
                Fun_ty.arg a (Fun_ty.arg b (Fun_ty.arg c (Fun_ty.last d))) )
      in
      let arg1_ty, ret_ty = Ty.domains ty in
      let arg2_ty, ret_ty = Ty.domains ret_ty in
      let arg3_ty, ret_ty = Ty.domains ret_ty in
    ...
    let function_4_adapter after sampler ty =
      let after =
        match after with
        | None -> fun _ _ _ -> []
        | Some after -> Fun_ty.apply_args_4 after
      in
      let pre, sampler, prot = function_4_adapter_pre sampler ty in
      (after, pre, sampler, prot)

    let test_function_4 ?test ?test_stdout ?test_stderr ?before ?after ty name
        tests =
      let tests =
        List.map
          (fun (w, x, y, z, r, out, err) ->
            ( Fun_ty.arg w @@ Fun_ty.arg x @@ Fun_ty.arg y @@ Fun_ty.last z
            , fun () -> output_string stdout out; output_string stderr err; r
            ) )
          tests
      in
    ...
    let test_function_4_against ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name rf tests =
      let tests =
        List.map
          (fun (w, x, y, z) ->
    ...
    let test_function_4_against_solution ?gen ?test ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler ty name tests =
      let tests =
        List.map
          (fun (w, x, y, z) ->
    ...
    let test_function_4_against_postcond ?gen ?test_stdout ?test_stderr
        ?before_reference ?before_user ?after ?sampler test ty name tests =
      let tests =
        List.map
          (fun (w, x, y, z) ->
      ...
      test_function_against_generic_postcond ?gen ?test_stdout ?test_stderr
        ~before_reference:(pre before_reference) ~before_user:(pre before_user)
        ~after ?sampler (Fun_ty.apply_args_4 test) name prot
        (lookup_student ty name) tests
  end
  ...
  module Sampler = struct
    ...
    let sample_char () = Char.chr (Random.int 26 + Char.code 'a')

    let sample_string () =
      let sample_piece () =
        [|"ba"; "be"; "4456"; ", "; "-"; " "; "OCaml"; "OCP"; "//"; "#"|].(Random
                                                                           .int
                                                                             10)
      in
    ...
    let sample_alternatively samplers =
      ...
      let samplers =
        if Random.bool () then Array.concat [samplers; samplers] else samplers
      in
    ...
    let sample_array ?(min_size = 0) ?(max_size = 10) ?(dups = true)
        ?(sorted = false) sample () =
      let sample =
        if dups then sample
        else
          ...
          let rec sample_new steps =
            if steps = 0 then invalid_arg "sample_array"
            else
              let s = sample () in
              try
                Hashtbl.find prev s;
                sample_new (steps - 1)
              with Not_found -> Hashtbl.add prev s (); s
          in
          fun () -> sample_new 100
      in
    ...
    let sample_pair sample1 sample2 () = (sample1 (), sample2 ())
    ...
    let fun_printer ppf f =
      let rec find = function
        | (f', n) :: rest ->
            if f == f' then Format.fprintf ppf "%s" n else find rest
        | [] -> Format.fprintf ppf "<fun>"
      in
      find !printable_funs

    let printable_fun n f =
      printable_funs := (Obj.repr f, n) :: !printable_funs;
      f

    let () =
      let path = Path.Pident (Ident.create "fun_printer") in
      let ty =
        Typetexp.transl_type_scheme !Toploop.toplevel_env
          (Ty.obj [%ty: _ -> _])
      in
  ...
  let ( @@= ) r1 f =
    if snd (Learnocaml_report.result r1) then r1 else r1 @ f ()
