...
let insert_in_env (type t) name (ty : t Ty.ty) (value : t) =
  ...
  let ty = Typetexp.transl_type_scheme !Toploop.toplevel_env (Ty.obj ty) in
  (Toploop.toplevel_env :=
     if String.uncapitalize_ascii name = name then
       ...
       | Ttyp_package {pack_type; _} ->
           Env.add_module (Ident.create name) pack_type !Toploop.toplevel_env
...
let insert_mod_ast_in_env ~var_name impl_code =
  ...
  let phr =
    ...
    @@ parse_mod_string (String.capitalize_ascii var_name) None impl_code
  in
  let open Parsetree in
  match phr with
  | Ptop_def
      [ { pstr_desc =
            Pstr_module {pmb_expr = {pmod_desc = Pmod_structure s; _}; _}; _ }
      ]
   |Ptop_def
      [ { pstr_desc =
            Pstr_module
              { pmb_expr =
                  { pmod_desc =
                      Pmod_constraint ({pmod_desc = Pmod_structure s; _}, _); _
                  }; _ }; _ } ] ->
      let ty =
        Ty.repr
          Ast_helper.(
            Typ.constr (Location.mknoloc (parse_lid "Parsetree.structure")) [])
      in
      insert_in_env var_name (ty : Parsetree.structure Ty.ty) s
  | _ (* should not happen *) -> assert false

let treat_lookup_errors fn =
  match fn () with
  | result -> result
  | exception Not_found -> Absent
  | exception Failure msg -> Incompatible msg
  ...
  | exception exn -> (
    ...
    | None ->
        Incompatible
          (Format.asprintf "%a@." Toploop.print_untyped_exception
             (Obj.repr exn))
...
let compatible_type nexp ngot =
  ...
  let texp =
    Ctype.newconstr path_exp
      (List.map (fun _ -> Ctype.newvar ()) decl_exp.Types.type_params)
  in
  let tgot =
    Ctype.newconstr path_got
      (List.map (fun _ -> Ctype.newvar ()) decl_got.Types.type_params)
  in
...
let get_value lid ty =
  treat_lookup_errors
  @@ fun () ->
  match (Ty.obj ty, (Longident.last lid).[0]) with
  | {Parsetree.ptyp_desc = Parsetree.Ptyp_package (n, rews); _}, 'A' .. 'Z' -> (
    match Env.lookup_module ~load:false lid !Toploop.toplevel_env with
    | exception Not_found -> Absent
    | path ->
        let {Types.md_loc; _} = Env.find_module path !Toploop.toplevel_env in
        let phrase =
          let open Ast_helper in
          with_default_loc md_loc
          @@ fun () ->
          let pack_expr =
            Exp.constraint_
          ...
          Parsetree.Ptop_def
            [ Str.value Asttypes.Nonrecursive
                [Vb.mk (Pat.var (Location.mkloc "%fake%" md_loc)) pack_expr] ]
        in
        let buf = Buffer.create 300 in
        let ppf = Format.formatter_of_buffer buf in
        if Toploop.execute_phrase false ppf phrase then
          let fake_path, _ =
            Env.lookup_value (Longident.Lident "%fake%") !Toploop.toplevel_env
          in
  ...
  | _ ->
      let {Typedtree.ctyp_type = exp_type; _} =
        Typetexp.transl_type_scheme !Toploop.toplevel_env (Ty.obj ty)
      in
      let path, {Types.val_type; _} =
        Env.lookup_value lid !Toploop.toplevel_env
      in
...
let print_value ppf v ty =
  ...
  let needs_parentheses =
    ...
    ( try
      ...
      with Exit -> () );
    match !state with
    | `Start | `Decided false | `Undecided -> false
    | `Decided true -> true
  in
  if needs_parentheses then (
...
let sample_value ty =
  ...
  let phrase =
    ...
    let sampler_id suffix =
      Exp.ident (Location.mknoloc (Longident.Lident ("sample_" ^ suffix)))
    in
    let rec phrase ty =
      match ty.desc with
      | Tconstr (path, [], _) -> sampler_id (Path.name path)
      | Tconstr (path, tl, _) ->
          Exp.apply
    ...
    Parsetree.Ptop_def
      [Str.value Nonrecursive [Vb.mk (Pat.var lid) (phrase ty)]]
  in
  ...
  | true ->
      let path, {Types.val_type; _} =
        Env.lookup_value (Longident.Lident lid) !Toploop.toplevel_env
      in
      let gty =
        Types.
          {ty with desc = Tarrow (Asttypes.Nolabel, Predef.type_unit, ty, Cok)}
      in
  ...
  | false -> failwith ("sampler could not be defined, " ^ Buffer.contents buf)
  ...
  | exception e ->
      failwith
        ( "error while defining sampler: " ^ Buffer.contents buf
        ^ Printexc.to_string e )

let register_callback name ty f =
  let unit =
    Ast_helper.(Typ.constr (Location.mknoloc (Longident.Lident "unit")) [])
  in
  let ty = Ty.curry ty (Ty.repr unit) in
  insert_in_env name ty f

let ref_lid = Location.mknoloc Longident.(Ldot (Lident "Pervasives", "ref"))
...
let allow_introspection ~divert =
  ...
  let module Introspection = struct
    type 'a t = 'a value = Absent | Present of 'a | Incompatible of string

    type 'a value = 'a t = Absent | Present of 'a | Incompatible of string
    ...
    let compatible_type name_exp name_got =
      compatible_type (parse_lid name_exp) (parse_lid name_got)
    ...
    let get_printer ty ppf v = print_value ppf v ty

    let get_sampler ty = sample_value ty

    let parse_lid name = parse_lid name
  end in
  insert_in_env "Introspection"
