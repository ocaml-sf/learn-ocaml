...
let token_type =
  ...
  function
  | COMMENT_OPEN_EOL | COMMENT_OPEN_CLOSE | COMMENT_OPEN | COMMENT_VERB_OPEN
   |COMMENT_CODE_OPEN | COMMENT_CONTENT | COMMENT_CLOSE | COMMENT_VERB_CLOSE
   |COMMENT_CODE_CLOSE ->
      "comment"
  | AND | AS | ASSERT | BEGIN | CLASS | CONSTRAINT | DO | DONE | DOWNTO
   |ELSE | END | EXCEPTION | EXTERNAL | FOR | FUN | FUNCTION | FUNCTOR | IF
   |IN | INCLUDE | INHERIT | INITIALIZER | LAZY | LET | MATCH | METHOD
   |MODULE | MUTABLE | NEW | OBJECT | OF | OPEN | OR | PRIVATE | REC | SIG
   ...
   |INFIXOP4 "asr" ->
      "keyword"
  | FLOAT _ | INT _ | INT32 _ | INT64 _ | NATIVEINT _ -> "constant"
  | INFIXOP0 _ | INFIXOP1 _ | INFIXOP2 _ | INFIXOP3 _ | INFIXOP4 _ | PREFIXOP _
    ->
      "function"
  | LABEL _ | OPTLABEL _ -> "type" (* Hack *)
  | AMPERAMPER | AMPERSAND | BACKQUOTE | BANG | BAR | BARBAR | BARRBRACKET
   |COLON | COLONCOLON | COLONEQUAL | COLONGREATER | COMMA | DOT | DOTDOT
   |EQUAL | GREATER | GREATERRBRACE | GREATERRBRACKET | LBRACE | LBRACELESS
   |LBRACKET | LBRACKETAT | LBRACKETATAT | LBRACKETATATAT | LBRACKETBAR
   |LBRACKETGREATER | LBRACKETLESS | LBRACKETPERCENT | LBRACKETPERCENTPERCENT
   |LESS | LESSMINUS | LPAREN | MINUS | MINUSDOT | MINUSGREATER | PLUS
   |PLUSDOT | QUESTION | QUESTIONQUESTION | QUOTE | RBRACE | RBRACKET
   |RPAREN | SEMI | SEMISEMI | SHARP | STAR | TILDE | UNDERSCORE ->
      "operator"
  | LINE_DIRECTIVE _ -> "meta"
  | FALSE | TRUE | LIDENT _ | UIDENT _ | TYPEVAR -> "variable"
  | EOL | SPACES -> "text"
  | ILLEGAL_CHAR _ -> "error"
  | EOF -> assert false
  | CHAR _ | STRING_OPEN | STRING_CONTENT | ESCAPED_EOL | STRING_CLOSE
   |PPX_QUOTATION_OPEN | PPX_QUOTATION_CONTENT | PPX_QUOTATION_CLOSE ->
      "string"
  | P4_QUOTATION_OPEN | P4_QUOTATION_CONTENT | P4_QUOTATION_CLOSE -> "meta"
...
let config =
  ref {indent = IndentConfig.{default with i_match_clause = 4}; forced = true}
...
let get_phrases doc =
  try List.assq doc !phrases with Not_found ->
    let ph = ref [] in
...
let get_line_tokens line st row doc =
  ...
  let rec iter ?(first = false) st offset stream tokens =
    ...
    | Some (tok, lex_ctxt, stream) -> (
        ...
        | EOL | ESCAPED_EOL -> (* FIXME some spaces ??? *)
                               (st, List.rev tokens)
        | COMMENT_OPEN_EOL ->
            (st, List.rev (comment_open tok.between :: tokens))
        | _ -> iter st offset stream (wrap_token st tok :: tokens) )
  in
...
let may_reset_indent ace_editor =
  let _, (row, _col) = Ace.read_range (Ace.get_selection_range ace_editor) in
  let line = Ace.get_line ace_editor row in
