...
let get_contents ?range {editor} =
  ...
  match range with
  | None -> Js.to_string @@ document##getValue
  | Some r -> Js.to_string @@ (document ## (getTextRange r))
...
let get_custom_data {editor} =
  match snd editor##.customData with None -> raise Not_found | Some x -> x

let set_custom_data {editor} data =
  let ed = fst editor##.customData in
  editor##.customData := (ed, Some data)

let set_mode {editor} name = editor ## getSession ## (setMode (Js.string name))
...
let show_keybindings e =
  ...
  | Some o -> o##showKeyboardShortcuts

let add_keybinding {editor} ?ro ?scrollIntoView ?multiSelectAction name key
    exec =
...
let set_font_size {editor} sz = editor ## (setFontSize sz)

let set_tab_size {editor} sz = editor ## getSession ## (setTabSize sz)

let set_readonly {editor} t = editor##setReadOnly (Js.bool t)

let get_state {editor} row = editor ## getSession ## (getState row)
