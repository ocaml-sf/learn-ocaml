...
let sync_dir =
  ...
  & info ["sync-dir"] ~docv:"DIR"
      ~doc:"Directory where to store user sync tokens"
...
let cert =
  ...
  & info ["cert"] ~docv:"BASENAME"
      ~env:(Arg.env_var "LEARNOCAML_CERT")
      ~doc:
...
let port =
  ...
  & info ["port"; "p"] ~docv:"PORT"
      ~env:(Arg.env_var "LEARNOCAML_PORT")
      ~doc:
        (Printf.sprintf
