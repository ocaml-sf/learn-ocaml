...
let readlink f =
  ...
  let f =
    ...
    with Sys_error _ -> f
  in
  try Sys.chdir cwd; f with Sys_error _ ->
...
module Args = struct
  ...
  let commands =
    value
    & pos_all
        (Arg.enum [("grade", Grade); ("build", Build); ("serve", Serve)])
        [Build; Serve]
  ...
  let app_dir =
    ...
    & info ["app-dir"; "o"] ~docv:"DIR"
        ~doc:
          "Directory where the app should be generated for the $(i,build) \
           command, and from where it is served by the $(i,serve) command."

  let base_url =
    value & opt string ""
    & info ["base-url"] ~docv:"BASE_URL"
        ~env:(Arg.env_var "LEARNOCAML_BASE_URL")
        ~doc:
          "Set the base URL of the website. Should not end with a trailing \
           slash. Currently, this has no effect on the backend - \
           '$(b,learn-ocaml serve)'. Mandatory for '$(b,learn-ocaml build)' \
           if the site is not hosted in path '/', which typically occurs for \
           static deployment."
  ...
  module Grader = struct
    ...
    let output_json =
      value
      & opt (some dir) None
      & info ["output-json"] ~docv:"DIR"
          ~doc:
            "save the processed exercises in JSON format in `.json` files, in \
             the given directory."

    let grade_student =
      value
      & opt (some file) None
      & info ["grade-student"; "s"] ~docv:"FILE"
          ~doc:"grade the given student file instead of 'solution.ml'"

    let display_outcomes =
      value & flag
      & info ["display-outcomes"] ~doc:"display the toplevel's outcomes"

    let quiet =
      value & flag
      & info ["quiet"; "q"] ~doc:"Don't display grading progression messages"

    let display_std_outputs =
      value & flag
      & info ["display-stdouts"] ~doc:"display the toplevel's standard outputs"

    let dump_outputs =
      value
      & opt (some string) None
      & info ["dump-outputs"] ~docv:"PREFIX"
          ~doc:"save the outputs in files with the given prefix"

    let dump_reports =
      value
      & opt (some string) None
      & info ["dump-reports"] ~docv:"PREFIX"
          ~doc:"save the reports in files with the given prefix"

    let timeout =
      value
      & opt (some int) None
      & info ["timeout"] ~docv:"SECONDS"
          ~doc:"Limit every test to the given timeout"

    let verbose =
      value & flag
      & info ["verbose"; "v"] ~doc:"Display detailed grading reports to stdout"
    ...
    let dump_dot =
      ...
      & info ["dump-dot"]
          ~doc:
            "Generates a dependency graph of the repository and dumps it into \
             the given file"

    type t = {exercises : string list; output_json : string option}
    ...
    let grader_cli =
      let apply grade_student display_outcomes quiet display_std_outputs
      ...
      in
      Term.(
        const apply $ grade_student $ display_outcomes $ quiet
  ...
  module Builder = struct
    ...
    let contents_dir =
      ...
      in
      value & opt dir default
      & info ["contents-dir"] ~docv:"DIR"
          ~doc:"directory containing the base learn-ocaml app contents"
    ...
    let playground =
      enable "playground"
        "the 'Playground' tab (enabled by default if the repository contains \
         a $(i,playground) directory)"
    ...
    let exercises =
      enable "exercises"
        "the 'Exercises' tab (enabled by default if the repository contains \
         an $(i,exercises) directory)"

    let toplevel = enable "toplevel" "the 'Toplevel' tab (enabled by default)"

    let exercises_filtered =
      value
      & opt_all (list string) [[]]
      & info
          ["exercises-filtered"; "f"]
          ~docv:"DIRS"
          ~doc:
            "Exercises to build (comma-separated), instead of taking the \
             entire repository. Can be repeated."

    let jobs =
      value & opt int 1
      & info ["jobs"; "j"] ~docv:"INT"
          ~doc:"Number of building jobs to run in parallel"
    ...
    let builder_conf =
      let apply contents_dir try_ocaml lessons exercises playground toplevel
          base_url =
        { contents_dir
        ; try_ocaml
        ; lessons
      ...
      in
      Term.(
        const apply $ contents_dir $ try_ocaml $ lessons $ exercises
        $ playground $ toplevel $ base_url)
  ...
  let term =
    ...
    in
    Term.(
      const apply $ commands $ app_dir $ repo_dir $ Grader.term $ Builder.term
      $ Server.term app_dir base_url)
end
...
let process_html_file orig_file dest_file base_url no_secret =
  let transform_tag e tag attrs attr =
    ...
    | Some url ->
        `Start_element
          ( (e, tag)
          , (attr_pair, base_url ^ url) :: List.remove_assoc attr_pair attrs )
    | None -> `Start_element ((e, tag), attrs)
  in
  Lwt_io.open_file ~mode:Lwt_io.Input orig_file
  >>= fun ofile ->
  Lwt_io.open_file ~mode:Lwt_io.Output dest_file
  >>= fun wfile ->
  let document = Markup_lwt.lwt_stream (Lwt_io.read_chars ofile) in
  Markup.parse_html document |> Markup.signals
  |> Markup.map (function
       | `Start_element ((e, "link"), attrs) ->
           transform_tag e "link" attrs "href"
       | `Start_element ((e, "script"), attrs) ->
           transform_tag e "script" attrs "src"
       | `Start_element ((e, "img"), attrs) ->
           transform_tag e "img" attrs "src"
       ...
       | t -> t )
  |> Markup.pretty_print |> Markup.write_html |> Markup_lwt.to_lwt_stream
  |> Lwt_io.write_chars wfile
  >>= fun () -> Lwt_io.close ofile >>= fun () -> Lwt_io.close wfile
  ...
  let grade () =
    if List.mem Grade o.commands then (
      if List.mem Build o.commands || List.mem Serve o.commands then
        failwith "The 'grade' command is incompatible with 'build' and 'serve'";
        ...
        (fun i ex ->
          let json_output =
            ...
            | Some o ->
                Some
                  (Filename.concat o
                     (String.map (function '/' -> '_' | c -> c) ex ^ ".json"))
          in
          Lwt.catch
            (fun () ->
            ...
            (fun e ->
              Printf.ksprintf failwith "Could not load exercise at %s: %s" ex
    ...
    if List.mem Build o.commands then (
      ...
      Lwt.catch
        ...
        (function
          | Failure _ ->
              Lwt.fail_with
              @@ Printf.sprintf "Failed to copy base app contents from %s"
      ...
      Lwt.catch
        ...
        (function
          | Unix.Unix_error (Unix.ENOENT, _, _) ->
              Lwt.return ServerData.empty_preconfig
          | exn -> Lwt.fail exn)
      >>= fun preconfig ->
      let json_config = ServerData.build_config preconfig in
      Learnocaml_store.write_to_file ServerData.config_enc json_config
        www_server_config
      >>= fun () ->
      ...
      |> Lwt_stream.iter_s (fun file ->
             ...
             if Filename.extension file = ".html" then
               process_html_file
                 (o.builder.Builder.contents_dir / file)
      ...
      let if_enabled opt dir f =
        ...
        | None ->
            Lwt.catch
              ...
              (function
                | Unix.Unix_error _ -> Lwt.return_false | e -> Lwt.fail e)
        | Some opt -> Lwt.return opt )
        >>= fun enabled ->
        if enabled then f dir >>= Lwt.return_some else Lwt.return_none
      in
      if_enabled o.builder.Builder.lessons (o.repo_dir / "lessons") (fun dir ->
          Lwt_utils.copy_tree dir (o.app_dir / "lessons")
          >>= fun () ->
          Lwt_unix.rename
            (o.app_dir / "lessons" / "lessons.json")
            (o.app_dir / "lessons.json")
          >|= fun () -> true )
      >>= fun lessons_ret ->
      ...
      Lwt_io.with_file ~mode:Lwt_io.Output
        (o.app_dir / "js" / "learnocaml-config.js")
        (fun oc ->
          Lwt_io.fprintf oc
            ...
            (tutorials_ret <> None) (playground_ret <> None)
            (lessons_ret <> None) (exercises_ret <> None)
            (o.builder.Builder.toplevel <> Some false)
            o.builder.Builder.base_url
          >>= fun () ->
          Lwt.return
            (tutorials_ret <> Some false && exercises_ret <> Some false) ) )
  ...
  let ret =
    Lwt_main.run
      ...
      | None ->
          ...
          if success then
            run_server ()
            >>= fun r -> if r then Lwt.return 0 else Lwt.return 10
          else Lwt.return 1 )
  in
  exit ret
...
let man =
  ...
  ; `S "COMMANDS"
  ; `P
      "The $(i,COMMAND) argument may be one or more of the following. If no \
       command is specified, '$(b,build) $(b,serve)' is assumed."
  ; `I ("$(b,grade)", "Runs the automatic grader on exercise solutions.")
  ; `I
      ( "$(b,build)"
      , "Generates the application based on a repository containing the \
         lessons, tutorials, playground and exercises (see $(b,REPOSITORY \
         FORMAT))." )
  ; `I
      ( "$(b,serve)"
      , "Run a web-server providing access to the learn-ocaml app, as well as \
  ...
  ; `S "REPOSITORY FORMAT"
  ; `P
      "The repository specified by $(b,--repo) is expected to contain \
       sub-directories $(b,lessons), $(b,tutorials), $(b,playground) and \
       $(b,exercises)."
  ; `S "AUTHORS"
  ; `P
      "The original authors are Benjamin Canou, Çağdaş Bozman, Grégoire \
       Henry and Louis Gesbert. It is licensed under the MIT License."
  ; `S "BUGS"
  ; `P
