...
module El = struct
  ...
  module Tabs = struct
    type t =
      {name : string; btn : Html_types.div H.elt; tab : Html_types.div H.elt}
...
let tab_select_signal, select_tab =
  ...
  let tab_select_signal, tab_select_signal_set = React.S.create !current in
  let select_tab el =
    let prev = !current in
    current := el;
  ...
  in
  List.iter
    (fun tab -> Manip.Ev.onclick tab.btn (fun _ -> select_tab tab; true))
    all;
...
let gather_assignments student_token index status =
  let status_map =
    List.fold_left
      (fun m ex -> SMap.add ex.Exercise.Status.id ex m)
      SMap.empty status
  in
  let assignments =
    get_assignments (Token.Set.singleton student_token) status_map
    |> List.filter (fun (_, toks, _, _) -> Token.Set.mem student_token toks)
  in
  let open_exercises =
  ...
  let assgs =
    List.map
      (fun ((start, stop), _tok, _dft, ids) -> (Some (start, stop), ids))
      assignments
  ...
  List.map
    (fun (dates, ids) ->
      ...
      let index = Exercise.Index.filter (fun id _ -> SSet.mem id ids) index in
      ( dates
      , List.rev
        @@ Exercise.Index.fold_exercises
             (fun l id meta ->
               if SSet.mem id ids then
                 ...
                 , ( match st with
                   | None -> []
                   | Some st -> Exercise.Status.skills_prereq meta st )
                 , match st with
                   | None -> []
                   | Some st -> Exercise.Status.skills_focus meta st )
                 :: l
               else l )
             [] index ) )
    assgs
...
let exercises_tab assignments answers =
  let grade_sty grade = H.a_style ("background-color:" ^ grade_color grade) in
  let exercise_line (id, meta, _, _) ans =
    ...
    let line =
      H.tr
        ~a:
          [ H.a_id (El.Dyn.exercise_line_id id)
      ...
      (fun (assg, ids) ->
        ...
        let text =
          ...
          | Some (start, _) when start > now ->
              [H.txt [%i "Future assignment (starting "]; date start; H.txt ")"]
          | Some (_, stop) when stop < now ->
              [H.txt [%i "Terminated assignment ("]; date stop; H.txt ")"]
          | Some (_, stop) ->
              [H.txt [%i "Ongoing assignment (due "]; date stop; H.txt ")"]
          | None -> [H.txt [%i "Open exercises"]]
        in
        ( H.tr
            ~a:
              [ H.a_class ["learnocaml-assignment-line"]
              ; grade_sty (Some (int_of_float avg_grade)) ]
            [ H.th ~a:[H.a_scope `Rowgroup; H.a_colspan 4] text
            ; H.th
                ~a:[H.a_scope `Rowgroup; H.a_class ["grade"]]
                [H.txt (Printf.sprintf "%01.1f%%" avg_grade)]
            ; H.th
                ~a:[H.a_scope `Rowgroup; H.a_class ["last-updated"]]
                [ ( match mtime with
                  | Some t -> date ~time:true t
                  | None -> H.txt "" ) ] ]
          :: lines
...
let stats_tab assignments answers =
  let smap_add n m key =
    ...
    with Not_found -> SMap.add key (n, 1) m
  in
  let total_grade, n_attempted, n_total, by_prereq, by_focus =
    List.fold_left
      (fun acc (_dates, exercises) ->
        List.fold_left
          (fun (total_grade, n_attempted, n_total, by_prereq, by_focus)
  ...
  let pct x y =
    ...
    if y = 0 then
      H.div
        ~a:[cls; H.a_style ("background-color:" ^ grade_color None)]
        [H.txt "--%"]
    else
      ...
      let background =
        Printf.sprintf
          "background:linear-gradient(to right,%s 0%%,%s %.0f%%,transparent \
           %.0f%%)"
          color color r r
      in
      H.div
        ~a:[H.a_class ["grade"; "stats-pct"]; H.a_style background]
        [H.txt (Printf.sprintf "%02.1f%%" r)]
  in
  [ H.h3 [H.txt [%i "Student stats"]]
  ; H.table
      ~a:[H.a_class ["student-stats"]]
      ( [ item [%i "completion"]
        ...
        ; item [%i "success"] [%i "The average grade over attempted exercises"]
            [pct total_grade (100 * n_attempted)] ]
      @ ( if SMap.is_empty by_focus then []
        else
          H.tr
            [ H.th ~a:[H.a_colspan 2]
      ...
      @
      if SMap.is_empty by_prereq then []
      else
        H.tr
          [ H.th ~a:[H.a_colspan 2]
...
let init_exercises_and_stats_tabs teacher_token student_token answers =
  ...
  exercises_tab assignments answers
  >|= fun (tbl, sighandlers) ->
...
let restore_report_button () =
  ...
  Manip.replaceChildren report_button Tyxml_js.Html5.[txt [%i "Report"]]
...
let display_report exo report =
  ...
  if grade >= 100 then (
    ...
    Manip.replaceChildren report_button
      Tyxml_js.Html5.[txt [%i "Report"]; span ~a:[a_class ["score"]] [txt pct]] );
  Manip.setInnerHtml
    El.Tabs.(report.tab)
...
let clear_tabs () =
  restore_report_button ();
  List.iter
    (fun t -> Manip.replaceChildren El.Tabs.(t.tab) [])
    El.Tabs.[report; text];
  clear_answer_tab ()
...
let () =
  ...
  let student_token =
    try Token.parse (List.assoc "token" Url.Current.arguments) with
    | Not_found | Failure _ -> failwith "Student token missing or invalid"
  in
  ...
  init_exercises_and_stats_tabs teacher_token student_token
  ...
  let _sig =
    ...
    |> React.S.map
       ...
       | Some ex_id ->
           ...
           update_tabs meta exo ans; Lwt.return_unit
  in
  Lwt.return_unit
