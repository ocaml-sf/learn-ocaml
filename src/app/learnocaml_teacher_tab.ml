...
let filter_input input_id list_id apply_fun =
  let input_field =
    H.input ~a:[H.a_id input_id; H.a_input_type `Search; H.a_list list_id] ()
  in
  Manip.Ev.oninput input_field (fun _ev ->
      apply_fun (Manip.value input_field);
      true );
  H.div
    ~a:[H.a_class ["filter_input"]]
...
let tag_addremove list_id placeholder add_fun remove_fun =
  let tag_input =
    H.input
      ~a:[H.a_input_type `Text; H.a_list list_id; H.a_placeholder placeholder]
      ()
  in
  let get_input_list () =
    List.filter (( <> ) "") (String.split_on_char ' ' (Manip.value tag_input))
  in
  H.div
    [ tag_input
    ; H.button
...
let rec teacher_tab token _select _params () =
  ...
  let action_csv_export () =
    ...
    >|= fun csv ->
    Learnocaml_common.fake_download ~name:"learnocaml.csv"
  ...
  let student_line_id student =
    "learnocaml_student_"
    ^ match student with `Any -> "any" | `Token tok -> Token.to_string tok
  in
  let assg_line_id id = "assg_line_" ^ string_of_int id in
  let all_exercises g =
    Exercise.Index.fold_exercises (fun acc id _ -> id :: acc) [] g |> List.rev
  in
  ...
  in
  let auto_checkbox () = H.div ~a:[H.a_class ["auto_checkbox"]] [] in
            ...
            let acc =
              H.tr
                ~a:
                  ...
                  ; H.a_onclick (fun _ ->
                        !toggle_selected_exercises all_children;
                        false ) ]
                [ H.td []
                ; H.th
                    ~a:[H.a_colspan 10; indent_style group_level]
          ...
          (fun acc (id, meta) ->
            ...
            let open_partition_ () =
              Lwt.async (fun () ->
                  ask_string ~title:"Choose a function name"
                    [ H.txt @@ "Choose a function name to partition codes from "
                      ^ id ^ ": " ]
                  >|= fun funname ->
                  let _win =
                    window_open
                      ( "/partition-view.html?id=" ^ id ^ "&function=" ^ funname
                      ^ "&prof=30" )
                      "_blank"
                  in
                  () );
            ...
            | Some meta ->
                ...
                let classes =
                  ...
                  @
                  if Hashtbl.mem selected_exercises id then ["selected"]
                  else []
                in
                ...
                H.tr
                  ~a:
                    ...
                    ; H.a_onclick (fun _ ->
                          !toggle_selected_exercises [id];
                          false )
                    ; H.a_ondblclick (fun _ -> open_exercise_ ())
                    ; H.a_onmouseup (fun ev ->
                          Js.Optdef.case ev##.which (fun () -> true)
                          @@ fun btn ->
                          if btn = Dom_html.Middle_button then
                            open_partition_ ()
                          else true ) ]
                  [ auto_checkbox_td ()
                  ...
                  ; H.td
                      ~a:[H.a_class ["skills-prereq"]]
                      ( List.map tag_span skills_prereq
                      @
                      if skills_prereq <> [] || skills_focus <> [] then
                        [H.txt "\xe2\x87\xa2"]
                        (* U+21E2, rightwards dashed arrow *)
                      else [] )
                  ; H.td
                      ~a:[H.a_class ["skills-focus"]]
                      (List.map tag_span skills_focus)
                  ; H.td [stars_div meta.Exercise.Meta.stars]
                  ; H.td
                      [ (let cls, text =
                           if Token.Map.is_empty ES.(st.assignments.token_map)
                           then
  ...
  let set_exercise_filtering str =
    let skills, keywords =
      List.partition
        (fun s -> SSet.mem s !all_exercise_skills)
        (List.filter (( <> ) "") (String.split_on_char ' ' str))
    in
    let res =
      List.map
        (fun s -> new%js Js.regExp_withFlags (Js.string s) (Js.string "i"))
        keywords
    in
    let matches id meta =
  ...
  let exercises_div =
    ...
    in
    H.div
      ~a:[H.a_id "exercises_pane"; H.a_class ["learnocaml_pane"]]
      [ H.div
          ~a:[H.a_id "exercises_filter_box"]
          [ H.datalist ~a:[H.a_id exercise_skills_list_id] ()
          ; filter_input "exercises_search_field" exercise_skills_list_id
              set_exercise_filtering ]
  ...
  let sort_to_str = function
    ...
    | `Tags -> "tags"
  in
  let str_to_sort s = List.find (fun t -> sort_to_str t = s) sort_type in
  ...
  in
  let html_token tk =
    H.span ~a:[H.a_class ["learnocaml_token"]] [H.txt (Token.to_string tk)]
  in
  let make_student_line ?(selected = false) st contents =
    let open_student_tab =
      let f t =
        let _win =
          window_open
            (Printf.sprintf "/student-view.html?token=%s" (Token.to_string t))
            "_blank"
        in
        false
      in
  ...
  let anystudents_line =
    make_student_line `Any
      [ H.td
          ~a:[H.a_colspan 10; H.a_class ["future_students"]]
  ...
  let fill_students_pane () =
    let compare =
      let open Student in
      let compare_nick st1 st2 =
        match (st1.nickname, st2.nickname) with
        | None, None -> 0
        | None, Some _ -> 1
        | Some _, None -> -1
        | Some n1, Some n2 ->
      ...
      | `Nick -> (
          fun st1 st2 ->
            match compare_nick st1 st2 with 0 -> compare st1 st2 | n -> n )
      | `Date -> (
          fun st1 st2 ->
    ...
    let table =
      ...
      :: List.map
           (fun st ->
             make_student_line
               ~selected:
                 (Hashtbl.mem selected_students (`Token st.Student.token))
               (`Token st.Student.token)
               [ H.td [html_token st.Student.token]
               ; H.td
                   ( match st.Student.nickname with
                   | Some n -> [H.txt n]
                   | _ -> [] )
               ...
               ; ( try find_component (student_progression_id st.Student.token)
                   with Failure _ ->
                     H.td
                       ~a:
                         [ H.a_id (student_progression_id st.Student.token)
    ...
    all_student_tags :=
      List.fold_left
        (fun tags st -> SSet.union tags st.Student.tags)
        SSet.empty all_students;
    Manip.replaceSelf
      (find_component student_tags_list_id)
      (H.datalist
         ~a:[H.a_id student_tags_list_id]
         ~children:
           (`Options
             (SSet.fold
                (fun tag acc -> H.option (H.txt tag) :: acc)
                !all_student_tags []))
         ())
  in
  let set_student_filtering str =
    let tags, keywords =
      List.partition
        (fun s -> SSet.mem s !all_student_tags)
        (List.filter (( <> ) "") (String.split_on_char ' ' str))
    in
    let res =
      List.map
    ...
    let hidden =
      if tags = [] && keywords = [] then (
        ...
        Manip.addClass anystudents_line "student_hidden";
        [`Any] )
    in
    let hidden =
      Token.Map.fold
  ...
  let change_student_tags f =
    ...
    fill_students_pane ();
    set_student_filtering (Manip.value (find_component "student_search_field"));
                ...
                let all =
                  List.filter
                    (fun t ->
                      not
                        (Manip.hasClass
                           (find_component (student_line_id t))
                           "student_hidden") )
                    all
                in
                !toggle_selected_students all;
                true ) ]
        [H.txt [%i "Students"]; H.txt " \xe2\x98\x90" (* U+2610 ballot box *)]
    in
    H.div
      ~a:[H.a_id "students_pane"; H.a_class ["learnocaml_pane"]]
      [ H.div
          ~a:[H.a_id "students_filter_box"]
          [ H.datalist ~a:[H.a_id student_tags_list_id] ()
          ; filter_input "student_search_field" student_tags_list_id
              set_student_filtering
          ; H.div ~a:[H.a_class ["filler_h"]] []
          ; H.label ~a:[H.a_label_for "student_sort"] [H.txt [%i "Sort by"]]
          ; H.select
              ~a:
                [ H.a_id "student_sort"
                ; H.a_oninput (fun _ev -> fill_students_pane (); true) ]
              [ H.option
                  ~a:[H.a_value (sort_to_str `Nick); H.a_selected ()]
                  (H.txt [%i "Nickname"])
              ; H.option
                  ~a:[H.a_value (sort_to_str `Token)]
                  (H.txt [%i "Token"])
              ; H.option
                  ~a:[H.a_value (sort_to_str `Date)]
                  (H.txt [%i "Creation date"])
              ; H.option ~a:[H.a_value (sort_to_str `Tags)] (H.txt [%i "Tags"])
              ] ]
      ; H.fieldset ~legend [students_list_div]
      ; H.div
          ~a:[H.a_id "student_controls"]
          [ tag_addremove student_tags_list_id [%i "tags"] add_student_tags
              remove_student_tags ] ]
  in
    ...
    all_exercise_skills :=
      Exercise.Index.fold_exercises
        (fun acc id meta ->
          ...
          let acc =
            List.fold_left
              (fun acc sk -> SSet.add sk acc)
              acc (ES.skills_prereq meta st)
          in
          List.fold_left
            (fun acc sk -> SSet.add sk acc)
            acc (ES.skills_focus meta st) )
        SSet.empty !exercises_index;
    Manip.replaceSelf
      (find_component exercise_skills_list_id)
      (H.datalist
         ~a:[H.a_id exercise_skills_list_id]
         ~children:
           (`Options
             (SSet.fold
                (fun skill acc -> H.option (H.txt skill) :: acc)
                !all_exercise_skills []))
         ());
  ...
  let assignment_line id =
            ...
            ~a:
              ( [ H.a_id ("date_" ^ id)
                ...
                ; H.a_onkeydown (fun ev ->
                      if ev##.keyCode = 13 then !assignment_change assg_id;
                      true )
        ...
        ; H.input
            ~a:
              ( [ H.a_id ("time_" ^ id)
                ...
                ; H.a_input_type `Time
                ; H.a_value
                    (Printf.sprintf "%02d:%02d" date##getHours date##getMinutes)
                ; H.a_onblur (fun _ -> !assignment_change assg_id; true)
                ; H.a_onkeydown (fun ev ->
                      if ev##.keyCode = 13 then !assignment_change assg_id;
                      true )
    ...
    let n_exos =
      match SSet.cardinal exo_ids with
      | 1 -> [%i "1 exercise"]
      | n -> Printf.sprintf [%if "%d exercises"] n
    in
    let n_students =
      match (Token.Set.cardinal tokens, default) with
      | 1, false -> [%i "1 student"]
    ...
    H.tr
        ...
        ; H.a_onclick (fun ev ->
              if
                Js.Opt.case ev##.target
                  (fun () -> true)
                  (fun e ->
                    String.lowercase_ascii (Js.to_string e##.tagName)
                    <> "input"
      ...
      ; H.td
          ~a:
            [ H.a_onclick (fun _ -> !assignment_remove id; false)
    ...
    List.fold_left
      (fun acc ex ->
        ...
        if
          ( default
          && ES.(
               match stat.assignments.default with
               | Assigned _ -> true
               | _ -> false) )
          || Token.Set.exists (is_assigned stat) students
        then SSet.add ex acc
        else acc )
      SSet.empty
  ...
  let assignments_table () =
    ...
    let new_assg_line =
      H.tr ~a:[H.a_id new_assg_id] [H.td ~a:[H.a_colspan 10] [new_assg_button]]
    in
    let new_assignment () =
      let start, stop =
      ...
      in
      let line = make_line (start, stop) tokens exercises default in
      let id = !line_n in
      Dom.insertBefore (H.toelt table) (H.toelt line)
    ...
    @@ List.map
         (fun (assg, tokens, dft, exos) -> make_line assg tokens exos dft)
         assignments
    @ [new_assg_line];
    table
  in
  let change_exercise_skills op of_meta of_status update_status =
    status_changes :=
      Hashtbl.fold
        ...
        selected_exercises !status_changes;
    fill_exercises_pane ();
    !update_changed_status ()
  in
  let add_requirements skills =
    change_exercise_skills (List.rev_append skills)
  ...
  let exercise_control_div =
    H.div
      ~a:[H.a_id "exercise_controls"]
      [ open_close_button
      ; H.div ~a:[H.a_class ["filler_h"]] []
      ; tag_addremove exercise_skills_list_id [%i "required skills"]
          add_requirements remove_requirements
      ; H.div ~a:[H.a_id "skills-arrow"] [H.txt "\xe2\x87\xa2"]
      ; (* U+21E2, rightwards dashed arrow *)
        tag_addremove exercise_skills_list_id [%i "trained skills"] add_focus
          remove_focus ]
  in
  ...
  let fill_control_div () =
    Manip.replaceSelf assignments_div (assignments_table ())
  in
  ...
  let apply_changes () =
    ...
    let students_changes_map =
      Token.Map.merge
        (fun tok std0 -> function None -> None
          | Some std ->
              let std0 =
                match std0 with Some s -> s | None -> Student.default tok
              in
    ...
    in
    ( if changes = [] then Lwt.return ()
    else retrieve (Learnocaml_api.Set_exercise_status (token, changes)) )
    >|= fun () ->
    ( if students_changes = [] then Lwt.return ()
    else retrieve (Learnocaml_api.Set_students_list (token, students_changes))
    )
    >|= fun () ->
  ...
  let actions_div =
    H.div ~a:[H.a_id "teacher_menubar"]
      ...
      ; H.button
          ...
          [H.txt [%i "Apply"]]
      ; dropdown ~id:"teacher-actions"
          ~title:[H.txt [%i "Actions"]]
          [ H.ul
              [ H.li
                  ~a:[H.a_onclick (fun _ -> Lwt.async action_new_token; true)]
                  [H.txt [%i "Create new teacher token"]]
              ; H.li
                  ~a:
                    [ H.a_onclick (fun _ ->
                          Lwt.async action_csv_export;
                          true ) ]
  ...
  let update_disabled_exercises () =
    ...
    let current_assignment =
      ...
      | Some id ->
          let _, _, exos, _ = Hashtbl.find assignments_tbl id in
          exos
      | None -> SSet.empty
    in
    let tokens, default = get_student_selection () in
    disabled
      (SSet.diff (already_assigned_exercises tokens default) current_assignment)
  in
  let set_assignment ?assg ?students ?exos ?default id =
    ...
    Hashtbl.replace assignments_tbl id ((start, stop), students, exos, default);
    ( match Manip.by_id (assg_line_id id) with
    ...
    let exercise_status_changes =
      SSet.fold
        (fun ex_id acc ->
          ...
          in
          let add tk st tmap =
            if st = new_default then tmap else Token.Map.add tk st tmap
          in
          let token_map =
            Token.Map.fold
              (fun tk _ acc ->
                if Token.Set.mem tk students then
                  if default then acc else Token.Map.add tk status acc
          ...
          in
          SMap.add ex_id
            ES.{st with assignments = {token_map; default = new_default}}
            acc )
        exos !status_changes
    in
    let exercise_status_changes =
      SSet.fold
        (fun ex_id acc ->
          ...
          in
          SMap.add ex_id
            ES.{st with assignments = {token_map; default = dft_status}}
            acc )
        (SSet.diff exos0 exos) exercise_status_changes
    in
  ...
  let update_disabled_students () =
    let set_enabled onoff student =
      ...
      if onoff then Manip.removeClass el "disabled"
      else (
        Manip.addClass el "disabled";
        Manip.removeClass el "selected" )
    in
    let disabled tokens =
      ...
      Token.Map.iter
        ...
        !students_map
    in
    let current_assignment, cur_default =
      match !selected_assignment with
      | Some id ->
          let _, std, _, dft = Hashtbl.find assignments_tbl id in
          (std, dft)
  ...
  (update_changed_status :=
     fun () ->
       if SMap.is_empty !status_changes && Token.Map.is_empty !students_changes
       then (
  ...
  (toggle_selected_exercises :=
     fun ?force ?(update = force = None) ids ->
       ...
       | Some aid ->
           set_assignment aid
             ~exos:(SSet.of_list (htbl_keys selected_exercises)) );
     ...
     fun ?force ?(update = force = None) students ->
       ...
       let students, onoff =
         ...
         | None ->
             ...
             in
             ( students
             , not @@ List.exists (Hashtbl.mem selected_students) students )
       in
  ...
  (toggle_select_assignment :=
     fun assg_id ->
       ...
       let select id =
         ...
         | Some line ->
             ...
             !toggle_selected_students ~force:false
               (Token.Map.fold
                  (fun tk _ acc -> `Token tk :: acc)
                  !students_map [`Any]);
             !toggle_selected_students ~force:true
               (Token.Set.fold
                  (fun tk acc -> `Token tk :: acc)
                  students
       ...
       update_disabled_both (); Lwt.return_unit);
  (exercise_status_change :=
     fun ids f ->
       ...
       update_disabled_exercises ();
       !update_changed_status ());
  (student_change := fun _tk () -> fill_students_pane ());
  (assignment_change :=
     fun assg_id ->
       ...
       let get_date id =
         let retr =
           ...
           | Some d, Some t -> (
             try
               Some
                 ( Scanf.sscanf (Manip.value d) "%d-%d-%d" (fun yr mon d ->
                       (yr, mon, d) )
                 , try
                     Scanf.sscanf (Manip.value t) "%d:%d" (fun hr mn -> (hr, mn)
                     )
                   with
                   | Scanf.Scan_failure _ | End_of_file -> (0, 0) )
             with
       ...
       in
       let start =
         match get_date ("start_" ^ assg_line_id assg_id) with
         | Some t -> t
         | None -> start0
       in
       let stop =
         match get_date ("stop_" ^ assg_line_id assg_id) with
  ...
  let fill_students_progression () =
    ...
    let students_assignments =
      ...
      in
      Hashtbl.fold
        (fun _ ((start, stop), tokens, exos, _dft) acc ->
      ...
      | _ :: _ as l ->
          ...
          Printf.sprintf "background:linear-gradient(to right,%s)"
            (String.concat ","
               (List.mapi
                  (fun i score ->
                    Printf.sprintf "%s %.0f%%,%s %.0f%%" (grade_color score)
                      (float_of_int i *. step)
                      (grade_color score)
                      (float_of_int (i + 1) *. step) )
                  l))
    in
    ...
    Token.Map.iter
      (fun tok st ->
        ...
        let grades exlist =
          List.map
            (fun ex ->
              match SMap.find_opt ex status with Some (_, g) -> g | _ -> None
              )
            exlist
        in
  ...
  let div =
    H.div
      ~a:[H.a_id "learnocaml-main-teacher"]
      [exercises_div; students_div; (* skills_div; *) control_div; actions_div]
  in
  let fetch_exercises =
  ...
  let fetch_stats =
    ...
    >|= fun statuses ->
    let map =
      List.fold_left (fun m ex -> SMap.add ex.ES.id ex m) SMap.empty statuses
    in
    status_map := map
  in
  let fetch_students =
    ...
    students_map :=
      List.fold_left
        (fun m st -> Token.Map.add st.Student.token st m)
        Token.Map.empty students
  in
  let content_div = find_component "learnocaml-main-content" in
  Manip.appendChild content_div div;
  Lwt.join [fetch_exercises; fetch_stats; fetch_students]
  >>= fun () ->
  exercises_index :=
    Exercise.Index.map_exercises
      (fun id meta ->
        let st = get_status id in
        Exercise.Meta.
          { meta with
          ...
          ; focus = ES.skills_base ~current:meta.focus st.ES.skills_focus } )
      !exercises_index;
  fill_exercises_pane ();
  (* fill_skills_pane (); *)
  fill_students_pane ();
