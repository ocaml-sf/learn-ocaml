...
let init_tabs, select_tab = mk_tab_handlers "text" ["text"; "meta"]
...
module Exercise_link = struct
  let exercise_link ?(cl = []) id content =
    ...
    Tyxml_js.Html5.(
      a
        ~a:[a_href ("/description/" ^ id ^ "#token=" ^ token); a_class cl]
        content)
end
...
let () =
  ...
  try
    ...
    let exercise_fetch = retrieve (Learnocaml_api.Exercise (Some token, id)) in
    init_tabs ();
    exercise_fetch
    >>= fun (ex_meta, exo, _deadline) ->
    (* display exercise questions *)
    let text_iframe = Dom_html.createIframe Dom_html.document in
    Manip.replaceChildren text_container
      Tyxml_js.Html5.
        [h1 [txt ex_meta.title]; Tyxml_js.Of_dom.of_iFrame text_iframe];
    Js.Opt.case
