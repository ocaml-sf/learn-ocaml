        ...
        List.fold_left
          (fun acc (Key storage_key) ->
            if storage_key.dependent_keys key then (
              ...
              if listeners = [] then acc else Key storage_key :: acc )
            else Key storage_key :: acc )
          [] !keys_with_listeners

let init () =
  let storage_event_typ : Dom_html.storageEvent Js.t Dom_html.Event.typ =
    Dom_html.Event.make "storage"
  in
  Dom_html.addEventListener Dom_html.window storage_event_typ
    (Dom_html.handler (fun evt ->
         Js.Opt.case evt##.key
           (fun () -> Js._false)
           (fun name ->
             let name = Js.to_string name in
             notify (Some name); Js._true ) ))
    Js._true
  |> ignore

let store {store; key; _} v = store v; notify key

let retrieve {retrieve; _} = retrieve ()

let delete {delete; key; _} = delete (); notify key

let listener key =
  let listener = ref None in
  if key.listeners = [] then
    keys_with_listeners := Key key :: !keys_with_listeners;
  key.listeners <- listener :: key.listeners;
  listener

let mangle parts = String.concat ":" ("learnocaml" :: parts)
    ...
    (fun localStorage ->
      Js.Opt.case
        ...
        (fun v ->
          ...
          try destruct enc Js._JSON ## (parse v) with exn ->
            raise
...
let delete_single name _enc () =
  Js.Optdef.case
    Dom_html.window##.localStorage
    (fun () -> failwith "local storage support required")
    (fun localStorage -> localStorage ## (removeItem (Js.string name)))

let clear () =
  Js.Optdef.iter Dom_html.window##.localStorage (fun localStorage ->
      localStorage##clear )
...
let nickname =
  ...
  and retrieve () = try retrieve_single key enc () with Not_found -> ""
  and delete () = delete_single key enc () in
  { key = Some key
  ; dependent_keys = ( = ) key
  ; store
...
let listed list_key item_prefix ?default enc =
  ...
  let item name =
    ...
    let store value =
      ...
      if not (List.mem name all) then store list (name :: all)
    and retrieve () = retrieve_single ?default key enc ()
    and delete () =
      delete_single key enc ();
      let all = retrieve list in
      if List.mem name all then store list (List.filter (( <> ) name) all)
    in
    { key = Some key
    ; dependent_keys = ( = ) key
    ; store
  ...
  let assoc =
    ...
    let dependent_keys name =
      ...
      let rec is_prefix p l =
        ...
        | [], [_] -> true
        | [], _ | _, [] -> false
        | pw :: p, lw :: l -> pw = lw && is_prefix p l
      in
      is_prefix item_prefix name
    in
    {key = None; dependent_keys; store; retrieve; delete; listeners = []}
  in
  (list, item, assoc)

let exercise_list, exercise_state, all_exercise_states =
  listed ["exercise-state-list"] ["exercise-state"] Answer.enc

let toplevel_history_list, toplevel_history, all_toplevel_histories =
  listed ["toplevel-history-list"] ["toplevel-history"]
