...
let save_file_enc =
  ...
  (obj5
     (dft "index"
        (map_enc Learnocaml_exercise_state.index_state_enc) StringMap.empty)
     (dft "editor"
        (map_enc Learnocaml_exercise_state.editor_state_enc) StringMap.empty)
     (dft "exercises"
        (map_enc Learnocaml_exercise_state.exercise_state_enc) StringMap.empty)
     (dft "toplevel-histories"
        (map_enc Learnocaml_toplevel_history.snapshot_enc) StringMap.empty)
     (dft "exercise-toplevel-histories"
        (map_enc Learnocaml_toplevel_history.snapshot_enc) StringMap.empty))
...
let sync
  ...
  let sync_snapshot snapshot_a snapshot_b =
    ...
    if snapshot_a.mtime > snapshot_b.mtime then
      ...
      snapshot_b in
  let sync_exercise_state (state_a :Learnocaml_exercise_state.exercise_state)
        (state_b : Learnocaml_exercise_state.exercise_state)  =
    let open Learnocaml_exercise_state in
    if state_a.mtime > state_b.mtime then
      state_a
    else
      state_b in
  let sync_editor_state (state_a:Learnocaml_exercise_state.editor_state)
        (state_b:Learnocaml_exercise_state.editor_state) =
    let open Learnocaml_exercise_state in
    if state_a.mtime > state_b.mtime then
      state_a
    else
      state_b in
  let sync_index_state (state_a :Learnocaml_exercise_state.index_state)
