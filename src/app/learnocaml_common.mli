...
val alert :
     ?title:string
  -> ?buttons:Html_types.div_content Tyxml_js.Html.elt list
  -> string
  -> unit
...
val catch_with_alert :
  ?printer:(exn -> string) -> (unit -> unit Lwt.t) -> unit Lwt.t
...
val disable_with_button_group :
  < disabled : bool Js.t Js.prop ; .. > Js.t -> button_group -> unit
...
val set_state_from_save_file : ?token:Token.t -> Save.t -> unit
...
val sync_exercise :
     Token.t option
  -> ?answer:Learnocaml_data.Answer.t
  -> ?editor:string
...
val exercise_text : Exercise.Meta.t -> Exercise.t -> string
...
val mouseover_toggle_signal :
  'a Tyxml_js.Html5.elt -> 'b -> ('b option -> unit) -> unit
...
val init_toplevel_pane :
  ...
  -> button_group
  -> (icon:string -> string -> (unit -> unit Lwt.t) -> unit)
  -> unit

val run_async_with_log : (unit -> 'a Lwt.t) -> unit

val mk_tab_handlers :
  string -> string list -> (unit -> unit) * (string -> unit)
...
val get_token :
     ?has_server:bool
  -> unit
  -> Learnocaml_data.student Learnocaml_data.token option Lwt.t
...
module Display_exercise (Q : sig
  ...
  val display_descr :
    Learnocaml_data.Exercise.Meta.t -> [> Html_types.div] Tyxml_js.Html5.elt
