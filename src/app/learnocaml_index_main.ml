    ...
    let rec format_contents lvl acc contents =
          ...
          List.fold_left
            (fun acc (exercise_id, meta_opt) ->
              ...
              | Some meta ->
                  ...
                  in
                  let time_left =
                    match List.assoc_opt exercise_id deadlines with
                    | None -> ""
                    | Some 0. -> [%i "Exercise closed"]
                    | Some f ->
                        Printf.sprintf [%if "Time left: %s"]
                  ...
                  a
                    ~a:
                      [ a_href
                          (get_url token "/exercises/" "exercise.html#id="
                             exercise_id)
                      ; a_class ["exercise"] ]
                    [ div
                        ~a:[a_class ["descr"]]
                        ( h1 [txt title]
                        ::
                        ( match short_description with
                        | None -> []
                        | Some text -> [txt text] ) )
                    ; div ~a:[a_class ["time-left"]] [H.txt time_left]
                    ; div
                        ~a:[Tyxml_js.R.Html5.a_class status_classes_signal]
                        [ stars_div stars
                        ; div
                            ~a:[a_class ["length"]]
                            [ ( match kind with
      ...
      | Exercise.Index.Groups groups ->
          ...
          List.fold_left
            ...
            acc groups
    in
    List.rev (format_contents 1 [] index)
  in
  let list_div =
    match
...
let playground_tab token _ _ () =
  show_loading [%i "Loading playground"]
  @@ fun () ->
  Lwt_js.sleep 0.5
  >>= fun () ->
  ...
  let list_div =
    let format_contents (id, pmeta) =
      ...
      a
        ~a:
          [ a_href (get_url token "/playground/" "playground.html#id=" id)
          ; a_class ["exercise"] ]
        [ div
            ~a:[a_class ["descr"]]
            ( h1 [txt title]
            ...
            | Some text -> [txt text] ) ) ]
    in
    List.map format_contents index
  in
...
let lessons_tab select (arg, set_arg, _delete_arg) () =
  ...
  Lwt_js.sleep 0.5
  >>= fun () ->
  retrieve (Learnocaml_api.Lesson_index ())
  >>= fun index ->
  let navigation_div = Tyxml_js.Html5.(div ~a:[a_class ["navigation"]] []) in
  let main_div = Tyxml_js.Html5.(div ~a:[a_class ["toplevel-pane"]] []) in
  let options =
    List.map
      (fun (lesson_id, lesson_title) ->
        ( lesson_id
        , Tyxml_js.Html5.(option ~a:[a_value lesson_id] (txt lesson_title)) )
        )
      index
  in
  let prev_and_next id =
    ...
    loop index
  in
  let selector = Tyxml_js.Html5.(select (snd (List.split options))) in
  ...
  let load_lesson ~loading () =
    ...
    retrieve ~ignore:Lesson.{title = ""; steps = []} (Learnocaml_api.Lesson id)
    >>= fun {Lesson.steps; _} ->
    ...
    toplevel_launch ~display_welcome:false main_div
      ...
      (fun () -> Lwt.async select)
      toplevel_buttons_group ("lesson-" ^ id)
    >>= fun top ->
    Lwt_list.iter_s
      (fun {Lesson.step_title; step_phrases} ->
        ...
        let do_phrase = function
          | Lesson.Text text ->
              ...
              Lwt.return ()
          | Lesson.Code code ->
              Learnocaml_toplevel.execute_phrase top code
              >>= fun _ -> Lwt.return ()
        in
        Lwt_list.iter_s do_phrase step_phrases >>= fun () -> Lwt.return () )
      steps
    >>= fun () ->
    set_arg "lesson" id;
  ...
  ( button ~group ~state:prev_button_state ~container:navigation_div
      ~theme:"black" ~icon:"left" [%i "Prev"]
  @@ fun () ->
  let selector = Tyxml_js.To_dom.of_select selector in
  let id = Js.to_string selector##.value in
  match prev_and_next id with
  ...
  := Dom_html.handler (fun _ ->
         Lwt.async (load_lesson ~loading:true);
         Js._true );
  ( button ~group ~state:next_button_state ~container:navigation_div
      ~theme:"black" ~icon:"right" [%i "Next"]
  @@ fun () ->
  let selector = Tyxml_js.To_dom.of_select selector in
  let id = Js.to_string selector##.value in
  match prev_and_next id with
  ...
  | _ -> Lwt.return () );
  let lesson_div =
    Tyxml_js.Html5.(div ~a:[a_id El.Dyn.lesson_id]) [navigation_div; main_div]
  in
  ...
  ( try
      let id =
        ...
        | id -> id
        | exception Not_found -> (
          match index with [] -> raise Not_found | (id, _) :: _ -> id )
      in
  ...
  >>= fun () -> Lwt.return lesson_div

let tryocaml_tab select (arg, set_arg, _delete_arg) () =
  let open Tutorial in
  let navigation_div = Tyxml_js.Html5.(div ~a:[a_class ["navigation"]] []) in
  let step_title_container = Tyxml_js.Html5.h3 [] in
  let step_title = Tyxml_js.Html5.span [] in
  let step_items_container = Tyxml_js.Html5.div [] in
  let step_div =
    Tyxml_js.Html5.(
      div
        ~a:[a_class ["step-pane"]]
        [step_title_container; step_items_container])
  in
  let toplevel_div = Tyxml_js.Html5.(div ~a:[a_class ["toplevel-pane"]] []) in
  let buttons_div = Tyxml_js.Html5.(div ~a:[a_class ["buttons"]] []) in
  let tutorial_div =
    Tyxml_js.Html5.(div ~a:[a_id El.Dyn.tryocaml_id])
      [navigation_div; step_div; toplevel_div; buttons_div]
  in
  let toplevel_buttons_group = button_group () in
  disable_button_group toplevel_buttons_group (* enabled after init *);
  let toplevel_launch =
    ...
    toplevel_launch ~on_disable ~on_enable toplevel_div
      Learnocaml_local_storage.toplevel_history
      (fun () -> Lwt.async select)
      toplevel_buttons_group "tryocaml"
  in
  ...
  retrieve ~ignore:[] (Learnocaml_api.Tutorial_index ())
  >>= fun index ->
  let index =
    List.flatten
    @@ List.fold_left
         (fun acc (_, {Index.series_tutorials; _}) -> series_tutorials :: acc)
         [] index
  in
  let options =
    List.map
  ...
  in
  let selector = Tyxml_js.Html5.(select (snd (List.split options))) in
  let dom_selector = Tyxml_js.To_dom.of_select selector in
  let prev_and_next id =
    let rec loop = function
      | [] -> assert false
      | [_] (* assumes single id *) -> (None, None)
      | {Tutorial.Index.name = one; _} :: {Tutorial.Index.name = two; _} :: _
        when id = one ->
          (None, Some two)
      | [{Tutorial.Index.name = one; _}; {Tutorial.Index.name = two; _}]
        when id = two ->
          (Some one, None)
      | {Tutorial.Index.name = one; _}
        :: {Tutorial.Index.name = two; _}
           :: {Tutorial.Index.name = three; _} :: _
        when id = two ->
          (Some one, Some three)
      | _ :: rest -> loop rest
    in
  ...
  let load_tutorial tutorial_name step_id () =
    retrieve
      ~ignore:{Tutorial.title = []; steps = []}
      (Learnocaml_api.Tutorial tutorial_name)
    >>= fun {Tutorial.steps; _} ->
    ...
    let option =
      Tyxml_js.To_dom.of_option (List.assoc tutorial_name options)
    in
    option##.selected := Js._true;
    let step =
      try List.nth steps step_id with _ -> failwith "unknown step"
    in
    if step_id = 0 then disable_button prev_step_button_state
    else enable_button prev_step_button_state;
    if step_id = List.length steps - 1 then
      disable_button next_step_button_state
    else enable_button next_step_button_state;
    current_tutorial_name := tutorial_name;
    current_step_id := step_id;
    Manip.replaceChildren step_title (render_rich_text step.step_title);
    let items =
      let on_runnable_clicked code =
        ...
        toplevel_launch
        >>= fun top ->
        if button_group_disabled toplevel_buttons_group then Lwt.return ()
        else
      ...
      let rec render_phrases phrases =
        List.map
          (function
            | Paragraph text ->
                Tyxml_js.Html5.p (render_rich_text ~on_runnable_clicked text)
            ...
            | Enum items ->
                Tyxml_js.Html5.ul
                  (List.map
                     (fun phrases -> Tyxml_js.Html5.li (render_phrases phrases))
                     items))
          phrases
      in
      render_phrases step.step_contents
    in
    ...
    Lwt.return ()
  in
  ( button ~group:toplevel_buttons_group ~state:prev_button_state
      ~container:navigation_div ~theme:"black" ~icon:"left" [%i "Prev"]
  @@ fun () ->
  match prev_and_next !current_tutorial_name with
  | Some prev, _ -> load_tutorial prev 0 ()
  | _ -> Lwt.return () );
  Manip.appendChild navigation_div selector;
  disable_with_button_group
    (Tyxml_js.To_dom.of_select selector)
  ...
  dom_selector##.onchange :=
    Dom_html.handler (fun _ ->
        ...
        Js._true );
  ( button ~group:toplevel_buttons_group ~state:next_button_state
      ~container:navigation_div ~theme:"black" ~icon:"right" [%i "Next"]
  @@ fun () ->
  match prev_and_next !current_tutorial_name with
  | _, Some next -> load_tutorial next 0 ()
  | _ -> Lwt.return () );
  ( button ~group:toplevel_buttons_group ~state:prev_step_button_state
      ~container:step_title_container ~theme:"black" ~icon:"left" ""
  @@ fun () -> load_tutorial !current_tutorial_name (!current_step_id - 1) ()
  );
  Manip.appendChild step_title_container step_title;
  ( button ~group:toplevel_buttons_group ~state:next_step_button_state
      ~container:step_title_container ~theme:"black" ~icon:"right" ""
  @@ fun () -> load_tutorial !current_tutorial_name (!current_step_id + 1) ()
  );
  load_tutorial !current_tutorial_name !current_step_id ()
  >>= fun () ->
  toplevel_launch
  >>= fun top ->
  let toplevel_button =
    button ~container:buttons_div ~theme:"dark" ~group:toplevel_buttons_group
      ?state:None
  in
...
let toplevel_tab select _ () =
  let container = Tyxml_js.Html5.(div ~a:[a_class ["toplevel-pane"]]) [] in
  let buttons_div = Tyxml_js.Html5.(div ~a:[a_class ["buttons"]]) [] in
  let div =
    Tyxml_js.Html5.(div ~a:[a_id El.Dyn.toplevel_id]) [container; buttons_div]
  in
  show_loading [%i "Launching OCaml"]
  @@ fun () ->
  let toplevel_buttons_group = button_group () in
  disable_button_group toplevel_buttons_group (* enabled after init *);
  toplevel_launch container Learnocaml_local_storage.toplevel_history
    (fun _ -> Lwt.async select)
    toplevel_buttons_group "toplevel"
  >>= fun top ->
  Manip.appendChild El.content div;
  let button =
    button ~container:buttons_div ~theme:"dark" ?group:None ?state:None
  in
...
let teacher_tab token a b () =
  ...
  @@ fun () ->
  Learnocaml_teacher_tab.teacher_tab token a b () >>= fun div -> Lwt.return div

let get_stored_token () = Learnocaml_local_storage.(retrieve sync_token)
...
let show_token_dialog token =
  ext_alert ~title:[%i "Your Learn-OCaml token"]
    [ H.p
        [ H.txt
            [%i
              "Your token is displayed below. It identifies you and allows to \
               share your workspace between devices."] ]
...
let init_token_dialog () =
  ...
  let create_token () =
    ...
    if Token.check nickname || String.length nickname < 2 then (
      ...
      retrieve (Learnocaml_api.Create_token (secret, None, Some nickname))
      >>= fun token ->
  ...
  let handler f t _ =
    Lwt.async (fun () ->
        f ()
        >|= function Some token -> Lwt.wakeup got_token token | None -> () );
    t
  in
...
let set_string_translations () =
  ...
  let translations =
    [ ("txt_connected_as", [%i "Connected as"])
    ; ("txt_choose_activity", [%i "Activities"])
    ; ( "txt_login_welcome"
      , configured config##.txtLoginWelcome [%i "Welcome to Learn OCaml"] )
  ...
  let placeholder_translations =
    [ (El.nickname_field, configured config##.txtNickname [%i "Nickname"])
    ; ( El.Login_overlay.input_nick
      , configured config##.txtNickname [%i "Nickname"] ) ]
  in
  List.iter
    (fun (el, text) ->
...
let () =
  ...
  let no_tab_selected () =
    ...
    let div =
      Tyxml_js.Html5.(div ~a:[a_class ["placeholder"]])
        Tyxml_js.Html5.[div [txt [%i "Choose an activity."]]]
    in
    Manip.removeChildren El.content;
    Manip.appendChild El.content div;
    delete_arg "activity"
  ...
  let init_tabs token =
    ...
    let tabs =
      ( if get_opt config##.enableTryocaml then
        [("tryocaml", ([%i "Try OCaml"], tryocaml_tab))]
      else [] )
      @ ( if get_opt config##.enableLessons then
          [("lessons", ([%i "Lessons"], lessons_tab))]
        else [] )
      @ ( if get_opt config##.enableExercises then
          [("exercises", ([%i "Exercises"], exercises_tab token))]
        else [] )
      @ ( if get_opt config##.enableToplevel then
          [("toplevel", ([%i "Toplevel"], toplevel_tab))]
        else [] )
      @ ( if get_opt config##.enablePlayground then
          [("playground", ([%i "Playground"], playground_tab token))]
        else [] )
      @
      match token with
        ...
        let rec select () =
          let th () =
            ...
            | None ->
                ...
                let delete_arg name =
                  ...
                  delete_arg name
                in
                callback select (arg, set_arg, delete_arg) ()
                >>= fun fresh ->
                div := Some fresh;
                Lwt.return_unit )
            >>= fun () ->
          ...
          Lwt.finalize
            ...
            (fun () ->
              ...
              Lwt.return_unit )
        in
        Manip.Ev.onclick btn (fun _ -> Lwt.async select; true);
  ...
  let download_save () =
    ...
    let contents =
      let json =
        Json_repr_browser.Json_encoding.construct Save.enc
          (get_state_as_save_file ~include_reports:true ())
      in
  ...
  in
  let import_save () =
    Learnocaml_common.fake_upload ()
    >>= fun (_, contents) ->
    let save_file =
      Json_repr_browser.Json_encoding.destruct Save.enc
        Js._JSON ## (parse contents)
    in
  ...
  let logout_dialog () =
    ...
    >|= (function
          | Ok _ -> [%i "Be sure to write down your token before logging out:"]
          | Error _ ->
              [%i
                "WARNING: the data could not be synchronised with the server. \
                 Logging out will lose your local changes, be sure you \
                 exported a backup."])
    >|= fun s ->
  ...
  List.iter
    (fun (text, icon, f) ->
      button ~container:El.sync_buttons ~theme:"white" ~group:sync_button_group
        ~icon text f )
    [ ( [%i "Show token"]
      , "token"
      , fun () ->
    ...
    ; ( [%i "Sync workspace"]
      , "sync"
      , fun () ->
          catch_with_alert @@ fun () -> sync () >>= fun _ -> Lwt.return_unit )
  ...
  ( button ~container:El.toolbar ~theme:"white" ~icon:"menu" [%i "Menu"]
  @@ fun () ->
  menu_hidden := not !menu_hidden;
  if !menu_hidden then Manip.addClass El.menu "hidden"
  else Manip.removeClass El.menu "hidden";
  Lwt.return () );
  ( ( try
  ...
  >>= (function
        ...
        | Error _ -> Lwt.return (init_tabs None))
  >>= fun tabs ->
  try
    let activity = arg "activity" in
    let _, select = List.assoc activity tabs in
    select ()
  with Not_found -> no_tab_selected (); Lwt.return ()
