...
let fake_download ~name ~contents =
  ...
  let url =
    Js.Unsafe.meth_call Js.Unsafe.global##._URL "createObjectURL"
...
let fatal ?(title = [%i "INTERNAL ERROR"]) message =
  ...
  let div =
    ...
    | None ->
        let div =
          H.div ~a:[H.a_id id; H.a_class ["learnocaml-dialog-overlay"]] []
        in
        Manip.(appendChild Elt.body) div;
        div
  in
  Manip.replaceChildren div
    [H.div [H.h3 [H.txt titletext]; H.div [H.p [H.txt (String.trim message)]]]]
...
let close_button txt = box_button txt @@ fun () -> ()
...
let ext_alert ~title ?(buttons = [close_button [%i "OK"]]) message =
  let div =
    ...
    | None ->
        let div =
          H.div
            ~a:[H.a_id dialog_layer_id; H.a_class ["learnocaml-dialog-overlay"]]
            []
        in
        Manip.(appendChild Elt.body) div;
        div
...
let lwt_alert ~title ~buttons message =
  ...
  let buttons =
    List.map
      (fun (txt, f) ->
        box_button txt (fun () ->
            Lwt.async @@ fun () -> f () >|= Lwt.wakeup_later wakener ) )
      buttons
  in
...
let confirm ~title ?(ok_label = [%i "OK"]) ?(cancel_label = [%i "Cancel"])
    contents f =
  ext_alert ~title contents
    ~buttons:[box_button ok_label f; close_button cancel_label]

let ask_string ~title ?(ok_label = [%i "OK"]) contents =
  let input_field = H.input ~a:[H.a_input_type `Text] () in
...
let show_loading ?(id = "ocp-loading-layer") contents f =
  let show () =
    ...
    let chamo_src =
      api_server ^ "/icons/tryocaml_loading_"
      ^ string_of_int (Random.int 9 + 1)
      ^ ".gif"
    in
  ...
  let hide () =
    ...
    Manip.(addClass elt "loaded")
  in
  Lwt.finalize (fun () -> show (); f ()) (fun () -> hide (); Lwt.return_unit)
...
let set_assoc name value =
  let rec set acc = function
    ...
    | arg :: args -> set (arg :: acc) args
  in
  set []

let delete_assoc name = List.filter (fun (n, _) -> n <> name)

let arg, set_arg, delete_arg =
  let args = ref (Js_utils.parse_fragment ()) in
  let delete_arg name =
    args := delete_assoc name !args;
    Js_utils.set_fragment !args
  in
...
let button_group () : button_group = (ref [], Lwt_mutex.create (), ref 0)

type button_state =
  bool ref * (button_group * < disabled : bool Js.t Js.prop > Js.t) option ref

let button_state () : button_state = (ref false, ref None)

let disable_button_group (buttons, _, cpt) =
  incr cpt;
  if !cpt = 1 then
    List.iter (fun (button, _) -> button##.disabled := Js.bool true) !buttons
...
let enable_button_group (buttons, _, cpt) =
  ...
  if !cpt = 0 then
    List.iter
      (fun (button, state) ->
        if not !state then button##.disabled := Js.bool false )
      !buttons
...
let enable_button (disabled, self) =
  ...
  | Some ((_, _, cpt), button) ->
      disabled := false;
      if !cpt = 0 then button##.disabled := Js.bool false

let button_group_disabled (_, _, cpt) = !cpt > 0

let disabling_button_group group cb =
  disable_button_group group;
  Lwt_js.yield ()
  >>= fun () ->
  Lwt.catch cb (function Lwt.Canceled -> Lwt.return () | exn -> Lwt.fail exn)
  >>= fun res ->
  enable_button_group group;
  Lwt_js.yield () >>= fun () -> Lwt.return res
...
let button ~container ~theme ?group ?state ~icon lbl cb =
  let ((others, mutex, cnt) as group) =
    match group with None -> button_group () | Some group -> group
  in
  let button =
    H.(
      button
        [ img ~alt:""
            ~src:(api_server ^ "/icons/icon_" ^ icon ^ "_" ^ theme ^ ".svg")
            ()
        ; txt " "
        ; span ~a:[a_class ["label"]] [txt lbl] ])
  in
  Manip.Ev.onclick button (fun _ ->
      ( Lwt.async
      @@ fun () ->
      Lwt_mutex.with_lock mutex @@ fun () -> disabling_button_group group cb );
      true );
  let dom_button =
    (Tyxml_js.To_dom.of_button button :> < disabled : bool Js.t Js.prop > Js.t)
  in
  let self_disabled =
    match state with
    | None -> ref false
  ...
  if !self_disabled || !cnt > 0 then dom_button##.disabled := Js.bool true;
    ...
    let disp =
      ...
      | _ ->
          Lwt_js_events.async (fun () ->
              Lwt_js_events.click window
              >|= fun _ -> Manip.SetCss.display menu "none" );
          "block"
    in
...
let render_rich_text ?on_runnable_clicked text =
  ...
  let rec render acc text =
    match text with
    | [] -> List.rev acc
    | Text text :: rest -> render (H.txt text :: acc) rest
    | Code {code; runnable} :: rest ->
        ...
        | _ -> () );
        render (elt :: acc) rest
    | Emph text :: rest -> render (H.em (render [] text) :: acc) rest
    | Image _ :: _ -> assert false
    | Math code :: rest -> render (H.txt ("`" ^ code ^ "`") :: acc) rest
  in
  (render [] text :> [< Html_types.phrasing > `Code `Em `PCDATA] H.elt list)
...
let extract_text_from_rich_text text =
  ...
  let rec render acc text =
    ...
    | Text text :: rest -> render (text :: acc) rest
    | Code {code; _} :: rest -> render (("[" ^ code ^ "]") :: acc) rest
    | Emph text :: rest -> render (("*" ^ render [] text ^ "*") :: acc) rest
    | Image {alt; _} :: rest -> render (("(" ^ alt ^ ")") :: acc) rest
    | Math code :: rest -> render (("$" ^ code ^ "$") :: acc) rest
  in
  render [] text
...
let get_state_as_save_file ?(include_reports = false) () =
  ...
  ; all_exercise_states = (if include_reports then answers else SMap.empty)
...
let rec sync_save token save_file =
  ...
  | Error (`Not_found _) ->
      Server_caller.request_exn
        (Learnocaml_api.Create_token ("", Some token, None))
      >>= fun _token ->
      assert (_token = token);
...
let sync_exercise token ?answer ?editor id =
  let handle_serverless () =
    ...
    | Some txt ->
        ...
        Learnocaml_local_storage.store key
          {a0 with Answer.solution = txt; mtime = gettimeofday ()}
    | None -> ()
  in
  ...
  | Some token ->
      Lwt.catch
        (fun () -> sync_save token save_file)
        (fun e -> handle_serverless (); raise e)
  | None ->
...
let string_of_seconds seconds =
  ...
  if days >= 1 then Printf.sprintf [%if "%dd %02dh"] days hours
  else if hours >= 1 then Printf.sprintf [%if "%02d:%02d"] hours minutes
  else Printf.sprintf [%if "0:%02d:%02d"] minutes seconds
...
let countdown ?(ontimeout = fun () -> ()) container t =
  ...
  let rec callback () =
    ...
    if remaining <= 0 then ( update 0; ontimeout () )
    else (
      update remaining;
      ignore
        (window##setTimeout
...
let flog fmt =
  Printf.ksprintf (fun s -> Firebug.console##log (Js.string s)) fmt
...
let exercise_text ex_meta exo =
  ...
  let descr =
    let lang = "" in
    try List.assoc lang Learnocaml_exercise.(access File.descr exo)
    with Not_found -> (
      try List.assoc "" Learnocaml_exercise.(access File.descr exo)
      with Not_found -> [%i "No description available for this exercise."] )
  in
  Format.asprintf
    "<!DOCTYPE html><html><head><title>%s - exercise text</title><meta \
     charset='UTF-8'><link rel='stylesheet' \
     href='%s/css/learnocaml_standalone_description.css'><script \
     type='text/x-mathjax-config'>%s</script>\n\
    \            <script type='text/javascript' \
     src='%s'></script></head><body>%s</body><script \
     type='text/javascript'>MathJax.Hub.Configured()</script></html>"
    ex_meta.Exercise.Meta.title api_server mathjax_config mathjax_url descr
...
let get_assignments tokens exos_status =
  ...
  let module ATM = Map.Make (struct
    ...
    let compare (d1, ts1, dft1) (d2, ts2, dft2) =
      ...
      | 0 -> (
        match Token.Set.compare ts1 ts2 with 0 -> compare dft1 dft2 | n -> n )
      | n -> n
  end) in
      ...
      (fun id st atm ->
        ...
        let atm =
          ...
          | ES.Assigned {start; stop} ->
              let explicit_tokens =
                Token.Map.fold
                  (fun tok _ -> Token.Set.add tok)
                  assg.ES.token_map Token.Set.empty
              in
              let implicit_tokens = Token.Set.diff tokens explicit_tokens in
              atm_add atm ((start, stop), implicit_tokens, true) id
          | _ -> atm
        in
        List.fold_left
          (fun atm (status, tokens) ->
            match status with
            ...
            | ES.Assigned {start; stop} -> (
                ...
                match ATM.find_opt key atm with
                | None -> ATM.add key (SSet.singleton id) atm
                | Some ids -> ATM.add key (SSet.add id ids) atm ) )
          atm stl )
      exos_status ATM.empty
  in
  ATM.fold
    (fun (assg, tokens, dft) exos l -> (assg, tokens, dft, exos) :: l)
    atm []
  |> List.rev
...
let string_of_date ?(time = false) t =
  ...
  if time then
    ...
    Printf.sprintf "%04d-%02d-%02d" date##getFullYear
      (date##getMonth + 1)
      date##getDate
...
let date ?(time = false) t =
  let date = new%js Js.date_fromTimeValue (t *. 1000.) in
  H.time
    ~a:[H.a_datetime (Js.to_string date##toISOString)]
    [ H.txt
        (Js.to_string
           (if time then date##toLocaleString else date##toLocaleDateString))
    ]
...
let tag_span tag =
  ...
  in
  H.span
    ~a:[H.a_class ["tag"]; H.a_style ("background-color: " ^ color)]
    [H.txt tag]

let get_worker_code name =
  let worker_url = ref None in
  fun () ->
    match !worker_url with
    | None ->
        retrieve (Learnocaml_api.Static ["js"; name])
        >|= fun js ->
        let url = js_code_url js in
        worker_url := Some url;
        url
...
let ace_display tab =
  let ace =
    lazy
      (let answer =
         Ocaml_mode.create_ocaml_editor (Tyxml_js.To_dom.of_div tab)
       in
       let ace = Ocaml_mode.get_editor answer in
       Ace.set_font_size ace 16; Ace.set_readonly ace true; ace)
  in
  ( (fun ans -> Ace.set_contents (Lazy.force ace) ~reset_undo:true ans)
  , fun () -> Ace.set_contents (Lazy.force ace) ~reset_undo:true "" )

let toplevel_launch ?display_welcome ?after_init ?(on_disable = fun () -> ())
    ?(on_enable = fun () -> ()) container history on_show
    toplevel_buttons_group id =
  let timeout_prompt = Learnocaml_toplevel.make_timeout_popup ~on_show () in
  let flood_prompt = Learnocaml_toplevel.make_flood_popup ~on_show () in
  let history =
    let storage_key = history id in
    let on_update self =
      Learnocaml_local_storage.store storage_key
        (Learnocaml_toplevel_history.snapshot self)
    in
    let snapshot = Learnocaml_local_storage.retrieve storage_key in
    Learnocaml_toplevel_history.create ~gettimeofday ~on_update ~max_size:99
      ~snapshot ()
  in
  get_worker_code "learnocaml-toplevel-worker.js" ()
  >>= fun worker_js_file ->
  Learnocaml_toplevel.create ~worker_js_file ?display_welcome ?after_init
...
let init_toplevel_pane toplevel_launch top toplevel_buttons_group
    toplevel_button =
  ( toplevel_button ~icon:"cleanup" [%i "Clear"]
  @@ fun () ->
  Learnocaml_toplevel.clear top;
  Lwt.return () );
  ( toplevel_button ~icon:"reload" [%i "Reset"]
  @@ fun () ->
  toplevel_launch
  >>= fun top ->
  disabling_button_group toplevel_buttons_group (fun () ->
      Learnocaml_toplevel.reset top ) );
  toplevel_button ~icon:"run" [%i "Eval phrase"]
  @@ fun () ->
...
let set_inner_list lst =
  let aux (id, text) =
    match Js_utils.Manip.by_id id with
    | None -> ()
    | Some component -> Manip.setInnerHtml component text
  in
  List.iter aux lst
...
let local_save ace id =
  ...
  in
  Learnocaml_local_storage.store key
    {ans with Answer.solution = Ace.get_contents ace; mtime = gettimeofday ()}
...
let mk_tab_handlers default_tab other_tabs =
  ...
  let select_tab name =
    ...
    Manip.removeClass
      ...
      "front-tab";
    Manip.enable (find_component ("learnocaml-exo-button-" ^ !current));
    Manip.addClass
      (find_component ("learnocaml-exo-button-" ^ name))
      "front-tab";
    Manip.addClass (find_component ("learnocaml-exo-tab-" ^ name)) "front-tab";
    Manip.disable (find_component ("learnocaml-exo-button-" ^ name));
    current := name
  in
  let init_tabs () =
    (current :=
...
module Editor_button (E : Editor_info) = struct
  ...
  let cleanup template =
    editor_button ~icon:"cleanup" [%i "Reset"]
    @@ fun () ->
    confirm ~title:[%i "START FROM SCRATCH"]
      [H.txt [%i "This will discard all your edits. Are you sure?"]]
      (fun () -> Ace.set_contents E.ace template);
    Lwt.return ()

  let download id =
    editor_button ~icon:"download" [%i "Download"]
    @@ fun () ->
    let name = id ^ ".ml" in
  ...
  let eval top select_tab =
    editor_button ~icon:"run" [%i "Eval code"]
    @@ fun () ->
    Learnocaml_toplevel.execute_phrase top (Ace.get_contents E.ace)
    >>= fun _ -> select_tab "toplevel"; Lwt.return_unit

  let sync token id =
    editor_button ~icon:"sync" [%i "Sync"]
    @@ fun () ->
    token
    >>= fun token ->
...
let setup_editor solution =
  ...
  let editor =
    Ocaml_mode.create_ocaml_editor (Tyxml_js.To_dom.of_div editor_pane)
  in
...
let typecheck top ace editor set_class =
  Learnocaml_toplevel.check top (Ace.get_contents ace)
  >>= fun res ->
  let error, warnings =
    match res with
  ...
  let error =
    ...
    | Some {Toploop_results.locs; msg; if_highlight} ->
        Some
          { Ocaml_mode.locs = List.map transl_loc locs
...
let setup_prelude_pane ace prelude =
  if prelude = "" then ()
    ...
    let state =
      ref
        ( match arg "prelude" with
        | exception Not_found -> true
        | "shown" -> true
        | "hidden" -> false
        | _ -> failwith "Bad format for argument prelude." )
    in
    let prelude_btn = button [] in
    let prelude_title = h1 [txt [%i "OCaml prelude"]; prelude_btn] in
    let prelude_container =
      pre
        ~a:[a_class ["toplevel-code"]]
        (Learnocaml_toplevel_output.format_ocaml_code prelude)
    in
    let update () =
      if !state then (
    ...
    in
    update ();
    Manip.Ev.onclick prelude_btn (fun _ ->
        state := not !state;
        update ();
        true );
    Manip.appendChildren prelude_pane [prelude_title; prelude_container]

let get_token ?(has_server = true) () =
  if not has_server then Lwt.return None
  else
    try Some Learnocaml_local_storage.(retrieve sync_token) |> Lwt.return
    with Not_found ->
      retrieve (Learnocaml_api.Nonce ())
      >>= fun nonce ->
      ask_string ~title:"Secret" [H.txt [%i "Enter the secret"]]
      >>= fun secret ->
      retrieve
        (Learnocaml_api.Create_token
           (Sha.sha512 (nonce ^ Sha.sha512 secret), None, None))
      >|= fun token ->
      Learnocaml_local_storage.(store sync_token) token;
      Some token

module Display_exercise (Q : sig
  val exercise_link :
       ?cl:string list
    -> string
...
struct
  ...
  let display_descr ex_meta =
    ...
    | Some descr ->
        div
          ~a:[a_class ["descr"]]
          [ h2
              ~a:[a_class ["learnocaml-exo-meta-category"]]
  ...
  let display_stars ex_meta =
    ...
    div
      ...
      [ p
          [ txt [%i "Difficulty:"]
          ; txt " "
          ; (* Put no whitespace in translation strings
  ...
  let display_exercise_meta id meta content_id =
    ...
    let descr =
      exercise_link ~cl:["exercise"] id
        [ display_descr meta
        ; H.div ~a:[] [display_stars meta; display_kind meta] ]
    in
  ...
  let display_list ?(sep = Tyxml_js.Html5.txt ", ") l =
    ...
    let rec gen acc = function
      | [] -> [txt ""]
      | [a] -> a :: acc
      | a :: (_ :: _ as rem) -> gen (sep :: a :: acc) rem
    in
    gen [] l |> List.rev
        ...
        Exercise.Index.fold_exercises
          (fun (req, focus) id meta ->
            let add sk id map =
              SMap.add sk
                (SSet.add id
                   (try SMap.find sk map with Not_found -> SSet.empty))
                map
            in
            ( List.fold_left
                (fun acc sk -> add sk id acc)
                req meta.Exercise.Meta.requirements
            , List.fold_left
                (fun acc sk -> add sk id acc)
                focus meta.Exercise.Meta.focus ) )
  ...
  let display_skill_meta _skill exs content_id =
    ...
    Manip.replaceChildren content
      ( display_list
      @@ List.map
           (fun ex_id -> exercise_link ex_id [Tyxml_js.Html5.txt ex_id])
           exs );
    Lwt.return ()
  ...
  let display_link onclick content_id value =
    ...
    let onclick _ =
      ...
      if !displayed then Manip.removeChildren (find_component cid)
      else ignore (onclick cid);
    ...
    div
      [ p
          ~a:[a_class ["learnocaml-exo-expandable-link"]; a_onclick onclick]
          [ span ~a:[a_id expand_id; a_class ["expand-sign"]] [txt "[+]"]
          ; txt value ]
      ; div ~a:[a_id cid; a_class ["learnocaml-exo-meta-category"]] [] ]
  ...
  let extract_maps_exo_index index =
    Exercise.Index.fold_exercises
      (fun (req, focus) id meta ->
        ( List.fold_left
            (fun acc sk -> add_map_set sk id acc)
            req meta.Exercise.Meta.requirements
        , List.fold_left
            (fun acc sk -> add_map_set sk id acc)
            focus meta.Exercise.Meta.focus ) )
  ...
  let opt_display_adjacent_exos index label exos caption =
    List.fold_left
      (fun acc id ->
        ...
        | Some meta -> display_exercise_link label meta id :: acc
        | None -> acc )
      [] (List.rev exos)
    |> function
    | [] -> None | l -> Some (caption, display_list ~sep:(H.txt "") l)
  ...
  let display_meta token ex_meta id =
    ...
    let authors =
      ...
      | authors -> Some (display_authors [%i "Authors:"] authors)
    in
           ...
           @ List.map
               (function
                 | Some (title, values) ->
                     div
                       ( h2
                           ~a:[a_class ["learnocaml-exo-meta-category-title"]]
                           [txt title]
                       :: values )
                 | None -> div [])
