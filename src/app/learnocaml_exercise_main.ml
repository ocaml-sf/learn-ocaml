...
let check_if_need_refresh has_server =
  if has_server then (
    ...
    if local_server_id <> server_id then
      let title =
        [%i "WARNING: You have an older grader version than the server"]
      and ok_label = [%i "Refresh the page"]
      and refresh () = Dom_html.window##.location##reload
      and cancel_label = [%i "I will do it myself!"]
      and message =
        [%i
          "The server has been updated, please refresh the page to make sure \
           you are using the latest version of Learn-OCaml server (none of \
...
module Exercise_link = struct
  let exercise_link ?(cl = []) id content =
    let open Tyxml_js.Html5 in
    a ~a:[a_href ("/exercises/" ^ id ^ "/"); a_class cl] content
end
...
let make_readonly () =
  ...
  alert ~title:[%i "TIME'S UP"]
    [%i
      "The deadline for this exercise has expired. Any changes you make from \
       now on will remain local only."]
...
let () =
  ...
  >>= fun has_server ->
  let token = get_token ~has_server () in
  (* ---- launch everything --------------------------------------------- *)
  ...
  let toplevel_button =
    button ~container:toplevel_toolbar ~theme:"dark"
      ~group:toplevel_buttons_group ?state:None
  in
  let id =
  ...
  let exercise_fetch =
    token >>= fun token -> retrieve (Learnocaml_api.Exercise (token, id))
  in
  let after_init top =
    exercise_fetch
    >>= fun (_meta, exo, _deadline) ->
    ( match Learnocaml_exercise.(decipher File.prelude exo) with
    | "" -> Lwt.return true
    | prelude ->
        Learnocaml_toplevel.load ~print_outcome:true top
          ~message:[%i "loading the prelude..."] prelude )
    >>= fun r1 ->
  ...
  in
  let toplevel_launch =
    toplevel_launch ~after_init
      (find_component "learnocaml-exo-toplevel-pane")
      Learnocaml_local_storage.exercise_toplevel_history
      (fun () -> select_tab "toplevel")
  ...
  let solution =
    ...
    | {Answer.report = Some report; solution; _} ->
        ...
        solution
    | {Answer.report = None; solution; _} -> solution
    | exception Not_found -> Learnocaml_exercise.(access File.template exo)
  in
  (* ---- details pane -------------------------------------------------- *)
  let load_meta () =
    Lwt.async (fun () -> token >>= fun token -> display_meta token ex_meta id)
  in
  if arg "tab" = "meta" then load_meta ()
  else
    Manip.Ev.onclick (find_component "learnocaml-exo-button-meta") (fun _ ->
        load_meta (); select_tab "meta"; true );
  (* ---- toplevel pane ------------------------------------------------- *)
  ...
  Manip.replaceChildren text_container
    Tyxml_js.Html5.
      [ h1 [txt ex_meta.Exercise.Meta.title]
      ; Tyxml_js.Of_dom.of_iFrame text_iframe ];
  (* ---- editor pane --------------------------------------------------- *)
  let editor, ace = setup_editor solution in
  let module EB = Editor_button (struct
    let ace = ace

    let buttons_container = editor_toolbar
  end) in
  ...
  ( toolbar_button ~icon:"list" [%i "Exercises"]
  @@ fun () ->
  ...
  let callback text =
    Manip.appendChild messages Tyxml_js.Html5.(li [txt text])
  in
  let worker = ref (get_grade ~callback exo) in
  (toolbar_button ~icon:"typecheck" [%i "Compile"] @@ fun () -> typecheck true);
  ( toolbar_button ~icon:"reload" [%i "Grade!"]
  @@ fun () ->
  check_if_need_refresh has_server
  >>= fun () ->
  let aborted, abort_message =
    ...
    let div =
      Tyxml_js.Html5.(
        div
          ~a:[a_class ["dialog"]]
          [txt [%i "Grading is taking a lot of time, "]; btn; txt " ?"])
    in
  ...
  | Toploop_results.Ok ((), _) ->
      let grading =
        Lwt.finalize
          (fun () ->
            !worker
            >>= fun w ->
            w solution >>= fun (report, _, _, _) -> Lwt.return report )
          (fun () ->
      ...
      let abortion =
        ...
        Lwt.return
          Learnocaml_report.
            [Message ([Text [%i "Grading aborted by user."]], Failure)]
      in
      Lwt.pick [grading; abortion]
      >>= fun report ->
      ...
      let editor, answer =
        ...
        else (Some solution, None)
      in
      token
      >>= fun token ->
      sync_exercise token id ?answer ?editor
      >>= fun _save ->
      select_tab "report";
      Lwt_js.yield () >>= fun () -> Ace.focus ace; Lwt.return ()
  | Toploop_results.Error _ ->
      let msg =
        Learnocaml_report.
          [ Text [%i "Error in your code."]
          ; Break
          ; Text
              [%i "Cannot start the grader if your code does not typecheck."]
          ]
      in
      ...
      Learnocaml_local_storage.(store (exercise_state id))
        ...
        ; mtime = gettimeofday () };
      select_tab "report";
      Lwt_js.yield () >>= fun () -> Ace.focus ace; typecheck true );
