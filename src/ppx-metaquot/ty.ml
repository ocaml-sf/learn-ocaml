...
let pair2 (Ty t1) (Ty t2) =
  Ty
    { Parsetree.ptyp_desc = Parsetree.Ptyp_tuple [t1; t2]
    ; ptyp_loc = Location.none
    ; ptyp_attributes = [] }

let pair3 (Ty t1) (Ty t2) (Ty t3) =
  Ty
    { Parsetree.ptyp_desc = Parsetree.Ptyp_tuple [t1; t2; t3]
    ; ptyp_loc = Location.none
    ; ptyp_attributes = [] }

let pair4 (Ty t1) (Ty t2) (Ty t3) (Ty t4) =
  Ty
    { Parsetree.ptyp_desc = Parsetree.Ptyp_tuple [t1; t2; t3; t4]
    ; ptyp_loc = Location.none
    ; ptyp_attributes = [] }
  ...
  Ty
    { Parsetree.ptyp_desc =
        Parsetree.Ptyp_constr
          ({Asttypes.txt = Longident.Lident "list"; loc = Location.none}, [ty])
    ; ptyp_loc = Location.none
    ; ptyp_attributes = [] }
